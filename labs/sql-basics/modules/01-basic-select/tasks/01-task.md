...
> Это задание повышенной сложности. Базовое задание — в файле `01-select.md`.
...
2. Найдите email-адреса всех пользователей, чья почта заканчивается на `@gmail.com`, без учета регистра символов (например, `@GMAIL.COM` или `@Gmail.Com` также должны попадать в выборку). Подсказка: используйте оператор `ILIKE`.
