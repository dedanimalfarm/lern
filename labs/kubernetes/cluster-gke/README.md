...
Terraform-описание дешёвого учебного GKE-кластера. Нужен потому, что
единственный «бесплатный» стенд (kwok-эмулятор GPU-нод) **не запускает
контейнеры** — на нём не отрабатывают `readinessProbe`, `logs`, `exec`,
StorageClass и т.д. Здесь — настоящий кластер, где лабы из `../modules`
проходят целиком.
