...
<!-- NAV -->
**⬅ [24-progressive-delivery](../24-progressive-delivery/)** · [индекс курса](../../README.md) · [карта обучения](../../docs/02-learning-path.md) · **[28-cost-multitenancy](../28-cost-multitenancy/) ➡**
<!-- /NAV -->
...
1. `base` vs `overlay` — кто на кого ссылается? Какая директива в yaml для этого используется?
   ...
   Overlay ссылается на base — через директиву `resources:` (раньше `bases:`, она устарела) в
   своём `kustomization.yaml`. Base ничего не знает об overlay'ях: он самодостаточен и может
   использоваться многими окружениями одновременно.
...
2. Какими трансформерами (блоками в `kustomization.yaml`) overlay меняет количество реплик у Deployment и тег образа?
   ...
   Реплики — трансформер `replicas:` (`- name: web, count: 3`) или стратегический патч
   (`patches:` с `patch: |` на `spec.replicas`). Тег образа — трансформер `images:`
   (`- name: nginx, newTag: 1.27-alpine`, при необходимости `newName`/`digest`).
...
3. В чём фундаментальная разница подходов Kustomize и Helm? Когда лучше использовать один, а когда другой?
   ...
   Kustomize — наложение патчей на готовый YAML: никаких шаблонов и языка, всё остаётся
   валидным Kubernetes-манифестом, diff читается глазами. Helm — шаблонизатор с параметрами,
   функциями, условиями, зависимостями и релизами (история, rollback). Kustomize удобнее для
   **своих** приложений с небольшими различиями между окружениями; Helm — для дистрибуции
   **чужим** пользователям и для сложной параметризации (чарты аддонов). Их часто сочетают:
   `kustomize build` поверх `helm template`.
...
4. Зачем в Kustomize нужна директива `namePrefix` и чем она опасна при обновлении существующего релиза в продакшене?
   ...
   `namePrefix` добавляет префикс ко всем именам ресурсов overlay'я — удобно разводить
   окружения в одном кластере (`dev-web`, `prod-web`). Опасность: имя — это идентичность
   объекта. Добавив (или изменив) префикс у работающего релиза, вы получите не переименование,
   а **создание новых объектов и удаление старых**: пересоздание Deployment, новые PVC (для
   StatefulSet — потеря привязки к данным), кратковременный простой.
...
5. В каком порядке применяются патчи, если их несколько для одного и того же ресурса?
   ...
   В порядке, в котором они перечислены в `patches:` (сверху вниз), и уже после применения
   `resources` и общих трансформеров. Последний патч, трогающий то же поле, побеждает —
   поэтому важен порядок, а не «сила» патча.
...
6. Что делает `list`-генератор и чем его можно заменить для автоматического сканирования каталогов в Git?
   ...
   `list`-генератор задаёт элементы явно, прямо в манифесте (`env: dev/staging/prod`). Для
   автоматики его заменяют `git`-генератором: `directories` (каждый подкаталог репозитория →
   своё Application) или `files` (каждый найденный `config.json` → элемент с его содержимым).
   Есть также `cluster`, `matrix`, `pullRequest` — для multi-cluster и preview-окружений.
...
7. Что произойдёт с объектами `Application` в кластере при добавлении нового или удалении существующего элемента в массиве генератора?
   ...
   ApplicationSet — контроллер: он приводит множество Application'ов к списку элементов
   генератора. Добавили элемент — появился новый Application (и развернулось окружение);
   удалили — соответствующий Application удаляется, а вместе с ним (каскадно, через finalizer)
   и его ресурсы. Именно поэтому правка генератора — операция такого же веса, как удаление
   окружения.
...
8. Зачем нужен AppProject? Перечислите 3 типа ограничений, которые в нем можно задать для защиты кластера.
   ...
   AppProject — граница доверия, чтобы одно скомпрометированное или ошибочное приложение не
   дотянулось до всего кластера. Ограничения: (1) `sourceRepos` — из каких репозиториев можно
   брать манифесты; (2) `destinations` — в какие кластеры и namespace можно деплоить;
   (3) `clusterResourceWhitelist`/`namespaceResourceBlacklist` — какие типы ресурсов разрешены
   (например, запретить создание ClusterRole). Плюс RBAC-роли внутри проекта.
...
9. Почему создание Namespace (`CreateNamespace=true` в `syncOptions`) требует добавления ресурса типа Namespace в `clusterResourceWhitelist` AppProject'а?
   ...
   Namespace — cluster-scoped ресурс, а по умолчанию проект разрешает только namespaced-объекты
   в разрешённых namespace. `CreateNamespace=true` заставляет Argo создать объект Namespace, и
   если его типа нет в `clusterResourceWhitelist`, sync падает с «resource Namespace is not
   permitted in project». Это защита: право создавать namespace — это право обойти квоты и
   политики, выданные на существующие namespace.
...
10. `selfHeal` vs `prune` — в чем разница? Приведите пример инцидента, когда может сработать одно, но не сработать другое.
   ...
   `selfHeal` возвращает **изменённые** объекты к состоянию git (кто-то сделал `kubectl scale` —
   реплики вернутся). `prune` удаляет объекты, которые **есть в кластере, но исчезли из git**.
   Пример: удалили манифест Service из репозитория — selfHeal не поможет (изменять нечего),
   сработает только prune; и наоборот, руками поменяли образ у живого Deployment — это чинит
   selfHeal, prune тут ни при чём.
...
11. Почему ручной `kubectl edit` или `kubectl scale` работающего окружения — антипаттерн в строгом GitOps подходе?
   ...
   Потому что кластер перестаёт соответствовать git: следующий sync (или selfHeal) молча
   затрёт правку, а если он не сработает — в репозитории останется неверное описание системы.
   Любое ручное изменение невоспроизводимо: его нет в истории, оно не пройдёт ревью и
   потеряется при пересоздании окружения. Правильный путь — коммит; `kubectl` оставляют для
   чтения и аварийных действий (и потом синхронизируют git с реальностью).
...
12. App-of-Apps vs ApplicationSet — в каких сценариях масштабирования вы выберете каждый из паттернов?
   ...
   App-of-Apps — «корневой» Application, в git которого лежат манифесты других Application:
   подходит, когда окружений немного и они **разные** (каждое со своим набором приложений),
   удобно версионировать состав платформы. ApplicationSet — генерация однотипных Application по
   шаблону: когда окружений/кластеров/команд много и они **однородны** (dev/staging/prod,
   десятки кластеров, preview на каждый PR). Их комбинируют: root-app разворачивает
   ApplicationSet'ы.
...
13. Что такое Flapping (бесконечный цикл синхронизации) в контексте Mutating Webhooks и `selfHeal`? Как директива `ignoreDifferences` помогает решить эту проблему?
   ...
   Flapping: мутирующий вебхук (service mesh, policy-engine) дописывает в объект поля, которых
   нет в git; Argo видит drift, `selfHeal` возвращает объект к git-версии, вебхук снова
   дописывает — и так бесконечно, с постоянными sync'ами и нагрузкой на apiserver.
   `ignoreDifferences` (по группе/типу/`jsonPointers` или `jqPathExpressions`) велит Argo не
   считать эти поля расхождением — цикл прерывается, а остальной drift по-прежнему
   отслеживается.
