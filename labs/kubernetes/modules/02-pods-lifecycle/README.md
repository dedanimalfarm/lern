...
> ⏱ время ~60 мин · сложность 2/5 · пререквизиты: модуль 01

<!-- NAV -->
**⬅ [01-kubectl-basics](../01-kubectl-basics/)** · [индекс курса](../../README.md) · [карта обучения](../../docs/02-learning-path.md) · **[03-workloads](../03-workloads/) ➡**
<!-- /NAV -->
...
1. Перечислите фазы Pod и состояния контейнера. Как они соотносятся?
   ...
   Фазы Pod (`.status.phase`): `Pending` (создан, но не все контейнеры запущены — ждёт
   scheduling, pull образа, mount тома), `Running` (назначен на ноду, хотя бы один контейнер
   работает), `Succeeded` (все завершились с 0 и не перезапускаются), `Failed` (все завершились,
   хотя бы один с ошибкой), `Unknown` (нет связи с kubelet). Состояния контейнера тоньше:
   `waiting` (с reason — `ContainerCreating`, `ImagePullBackOff`, `CrashLoopBackOff`), `running`,
   `terminated` (с `exitCode` и `reason`). Фаза агрегирует состояния контейнеров, поэтому
   `Running` ничего не говорит о готовности — для этого есть conditions.
...
2. Объясните на примере, почему Pod может быть `Running` при `READY 0/1`.
   ...
   `READY n/m` считает контейнеры, прошедшие **readinessProbe**. Контейнер запущен (фаза
   `Running`), но приложение внутри ещё прогревается или отвечает не тем, что ждёт проба —
   condition `Ready=False`, `READY 0/1`, и под не попадает в Endpoints. Классика: проба стучится
   в `/healthz`, а приложение отдаёт только `/`.
...
3. Как `restartPolicy` меняет поведение завершившегося контейнера?
   ...
   `Always` (по умолчанию для Pod из Deployment) — перезапускать всегда, даже после exit 0;
   именно отсюда `CrashLoopBackOff` с растущей паузой. `OnFailure` — только при ненулевом коде
   выхода (типично для Job). `Never` — не перезапускать; упавший под остаётся в `Failed`, а Job
   создаёт вместо него **новый** под.
...
4. Зачем init-контейнеры выделены отдельно? Приведите два реальных применения.
   ...
   Init-контейнеры выполняются последовательно и **до** основных, каждый должен завершиться
   успешно. Это даёт гарантию «подготовка закончена до старта приложения» и позволяет держать в
   init-образе инструменты, которых нет (и не должно быть) в рантайм-образе. Примеры: дождаться
   доступности БД или миграции схемы; скачать/сгенерировать конфиг или сертификат в общий
   `emptyDir`, который потом читает приложение.
...
5. Что будет с Pod, если один из нескольких init-контейнеров упадёт?
   ...
   Pod не перейдёт к следующим init-контейнерам и к основным: kubelet перезапускает упавший
   init согласно `restartPolicy` (при `Always`/`OnFailure` — бесконечно, с back-off). Под висит
   в `Pending`/`Init:CrashLoopBackOff`, condition `Initialized=False`. В `kubectl get pod` это
   видно как `Init:0/2`.
...
6. Чем init-контейнер отличается от sidecar?
   ...
   Обычный init завершается до старта приложения и блокирует его. Sidecar работает
   **параллельно** с приложением всю его жизнь (прокси, лог-шиппер, агент). С 1.29 sidecar
   оформляют как init-контейнер с `restartPolicy: Always`: он стартует до приложения, не
   блокирует переход к нему (достаточно `Started`) и гасится после него — поэтому Job с таким
   sidecar корректно завершается (модуль 29).
...
7. Сведите в таблицу различия `readiness`/`liveness`/`startup`: что проверяют,
   ...
   | Проба | Что проверяет | Провал |
   |---|---|---|
   | `readiness` | готов ли принимать трафик | под убирается из Endpoints, рестарта нет |
   | `liveness` | жив ли процесс (не завис) | kubelet **убивает** контейнер, он перезапускается |
   | `startup` | закончился ли долгий старт | контейнер убивается; пока startup не прошёл, liveness и readiness не опрашиваются |
...
8. Почему агрессивная `livenessProbe` — частая причина рукотворных аварий?
   ...
   Потому что она перезапускает **здоровое** приложение. Короткий `periodSeconds`, маленький
   `failureThreshold` и тяжёлый health-эндпоинт превращают временное замедление (GC, всплеск
   нагрузки, медленный запрос к БД) в убийство контейнера — а рестарт делает нагрузку на
   оставшиеся реплики ещё выше, и они падают следом. Признак: рестарты при пустых логах
   ошибок и `Liveness probe failed` в событиях (сценарий 02 модуля 08).
...
9. Когда `startupProbe` обязателен, а `initialDelaySeconds` недостаточно?
   ...
   Когда время старта непредсказуемо и может сильно превышать обычное: миграции, прогрев кэша,
   JIT. `initialDelaySeconds` — фиксированная задержка: поставите мало — liveness убьёт при
   медленном старте, поставите много — реальная авария будет обнаружена с опозданием.
   `startupProbe` даёт **бюджет** (`failureThreshold × periodSeconds`) только на старт и, пройдя
   один раз, больше не мешает: liveness дальше работает со своими строгими таймингами.
...
10. Как вычисляется QoS-класс? Дайте конфиг на `Guaranteed`, `Burstable`,
   ...
   `Guaranteed` — у **каждого** контейнера заданы requests и limits и они равны по CPU и памяти;
   `Burstable` — что-то задано, но условие Guaranteed не выполняется; `BestEffort` — не задано
   ничего.
   ```yaml
   resources: { requests: {cpu: 100m, memory: 64Mi}, limits: {cpu: 100m, memory: 64Mi} }  # Guaranteed
   resources: { requests: {cpu: 100m, memory: 64Mi}, limits: {cpu: 500m, memory: 128Mi} } # Burstable
   # (блок resources отсутствует)                                                          # BestEffort
   ```
...
11. Почему `OOMKilled` происходит на уровне cgroup, а не всей ноды?
   ...
   `limits.memory` транслируется в `memory.max` cgroup контейнера. Когда процессы внутри
   превышают лимит, OOM-killer ядра срабатывает **внутри этой cgroup** и убивает процесс
   контейнера, не трогая остальную ноду. Отсюда `reason: OOMKilled`, `exitCode: 137` и рестарт
   одного пода при здоровой ноде. Глобальный OOM (нехватка памяти на всей ноде) — другой случай:
   там kubelet заранее начинает **выселять** поды (`Evicted`).
...
12. В каком порядке вытесняются поды при memory pressure и почему именно так?
   ...
   При memory pressure kubelet выселяет в порядке: сначала `BestEffort`, затем `Burstable`,
   потребляющие **сверх** своих requests (чем больше превышение относительно requests, тем
   раньше), в последнюю очередь `Guaranteed`. Логика — сначала убрать тех, кто ничего не
   зарезервировал и на кого никто не рассчитывал; `Guaranteed`-под получил гарантию и забирает
   ровно столько, сколько просил.
