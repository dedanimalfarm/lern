...
<!-- NAV -->
**⬅ [20-batch-workflows](../20-batch-workflows/)** · [индекс курса](../../README.md) · [карта обучения](../../docs/02-learning-path.md) · **[22-ingress-tls](../22-ingress-tls/) ➡**
<!-- /NAV -->
...
1. В чём ключевое архитектурное отличие деплоя базы данных через Operator по сравнению со стандартным Kubernetes StatefulSet?
   ...
   StatefulSet даёт только примитивы: стабильные имена, порядок запуска и по PVC на реплику.
   Он не знает, что внутри — PostgreSQL или Redis, поэтому все доменные операции (кто primary,
   как переключить, как снять бэкап, как обновить версию без потери данных) остаются человеку.
   Оператор — это CRD + контроллер, знающий доменную логику: он сам выбирает primary, следит за
   репликацией, делает failover, бэкапы и rolling upgrade, а вы описываете намерение
   (`instances: 3`) вместо процедуры.
...
2. Какие человеческие задачи (DBA) берет на себя оператор?
   ...
   Первичная инициализация кластера (initdb, пользователи, БД), настройка и мониторинг
   репликации, failover и switchover с защитой от split-brain, бэкапы и PITR, обновление
   minor/major версий, масштабирование реплик чтения, пулинг соединений (Pooler), ротация
   сертификатов и паролей, сбор метрик для Prometheus.
...
3. Зачем CloudNativePG запускает бинарник `Instance Manager` (PID 1) вместо самого процесса `postgres`?
   ...
   Instance Manager — это supervisor PostgreSQL внутри пода: он запускает и останавливает
   `postgres`, отвечает на probes, применяет конфигурацию, участвует в выборах и выполняет
   команды оператора (promote, switchover, backup), а также корректно завершает БД при
   остановке пода. Если бы PID 1 был самим `postgres`, у оператора не было бы точки управления
   внутри контейнера, а сигналы Kubernetes приходили бы напрямую СУБД без предварительных
   действий.
...
4. Какие Services автоматически создаются для маршрутизации трафика и в чем разница между `rw` и `ro`?
   ...
   `<cluster>-rw` — на текущий primary (чтение и запись), `<cluster>-ro` — только на реплики
   (чтение), `<cluster>-r` — на все инстансы (чтение с любого, включая primary). Приложение
   пишет через `-rw`: при failover сервис автоматически переезжает на нового primary, менять
   строку подключения не нужно. Попытка писать через `-ro` даёт
   `cannot execute ... in a read-only transaction` (сценарий 02).
...
5. Почему для баз данных важно использовать механизмы обеспечения кворума? Как CNPG решает проблему Split-Brain?
   ...
   Кворум нужен, чтобы исключить ситуацию, когда два узла одновременно считают себя primary и
   принимают запись — это расхождение данных, которое невозможно автоматически слить. CNPG не
   строит собственный консенсус: он опирается на Kubernetes API (etcd уже даёт консенсус),
   хранит состояние в объекте `Cluster` и использует механизм lease. Старый primary, вернувшись
   после сбоя, видит в API, что primary уже другой, и переходит в роль реплики (fencing).
...
6. Что произойдет с кластером PostgreSQL, если удалить под с самим оператором `cnpg-controller-manager`? Перестанет ли БД отвечать на запросы?
   ...
   База продолжит работать: оператор нужен для управляющих операций, а трафик идёт напрямую
   через Services к подам PostgreSQL. Перестанут выполняться failover, масштабирование, бэкапы
   по расписанию и применение изменений `Cluster` — то есть кластер станет неуправляемым, но не
   недоступным. Deployment оператора при этом сам поднимет новый под.
...
7. В чём разница между CRD `Backup` и `ScheduledBackup`?
   ...
   `Backup` — одноразовая операция «сделать бэкап сейчас» (создали объект → оператор выполнил →
   статус `completed`). `ScheduledBackup` — расписание в cron-формате, которое само создаёт
   объекты `Backup` в нужное время, плюс политика хранения. Оба требуют настроенного
   назначения: на стенде `spec.backup` у кластера пуст, поэтому ручной `Backup` честно падает
   (см. Часть 4 модуля).
...
8. Зачем в ресурсе `Cluster` используется параметр `bootstrap: recovery`, и в каких сценариях он применяется (что такое PITR)?
   ...
   `bootstrap: recovery` означает «создать новый кластер не с нуля (`initdb`), а восстановив
   данные» — из бэкапа, из объектного хранилища или напрямую с работающего кластера
   (`pg_basebackup`). PITR (Point-In-Time Recovery) — восстановление на **конкретный момент**:
   берётся базовый бэкап и поверх накатываются архивированные WAL до указанного
   `recoveryTarget.targetTime`. Применяется после логических ошибок (`DROP TABLE`, кривая
   миграция), когда нужно вернуть состояние «за минуту до».
...
9. Если под БД уходит в `CrashLoopBackOff`, какую команду вы используете первой для диагностики?
   ...
   `kubectl -n lab logs <pod> -c postgres --previous` — логи упавшего экземпляра (там обычно
   причина: не смог примонтировать том, повреждён WAL, не хватило памяти, конфликт версии
   данных). Затем `kubectl -n lab describe pod <pod>` (события, OOMKilled, проблемы с PVC) и
   `kubectl -n lab get cluster my-db -o yaml` — статус и сообщения оператора.
...
10. Почему добавление PgBouncer (ресурс `Pooler`) спасает кластер от Out-Of-Memory ошибок при большом количестве микросервисов?
   ...
   Каждое соединение PostgreSQL — это отдельный процесс ОС с собственной памятью (work_mem и
   пр.). Сотня микросервисов со своими пулами легко даёт тысячи соединений — БД уходит в OOM
   или упирается в `max_connections`. PgBouncer (ресурс `Pooler` в CNPG) держит небольшое число
   реальных соединений к БД и мультиплексирует в них клиентские (transaction pooling), поэтому
   потребление памяти не растёт линейно с числом клиентов.
