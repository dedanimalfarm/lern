...
- **Под БД в статусе `CrashLoopBackOff` или `Error`**:
  Проверьте логи пода с помощью `kubectl logs -n lab my-db-1`. Часто причина кроется в нехватке ресурсов или неправильной конфигурации `postgresql.conf`.
- **Primary не переключается после падения (Failover не работает)**:
  Убедитесь, что оператор запущен и работает: `kubectl -n cnpg-system get pods`. Посмотрите его логи. Если оператор упал, он не сможет управлять кластером.
- **Бэкапы не создаются**:
  Проверьте статус ресурса `ScheduledBackup` (`kubectl -n lab get scheduledbackup`). Если он сообщает об ошибке, убедитесь, что конфигурация подключения к S3-совместимому хранилищу указана верно и креды (секреты) доступны в том же неймспейсе.
...
1. В чём ключевое архитектурное отличие деплоя базы данных через Operator по сравнению со стандартным ресурсом Kubernetes StatefulSet?
2. Почему для баз данных важно использовать механизмы обеспечения кворума, и как CloudNativePG решает проблему Split-Brain?
3. Что произойдет, если удалить под с оператором CloudNativePG во время нормальной работы кластера PostgreSQL?
4. Зачем в ресурсе `Cluster` используется параметр `bootstrap: recovery`, и в каких сценариях он применяется?
