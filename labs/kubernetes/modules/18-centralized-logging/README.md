...
| Свежие логи приходят с задержкой в минуты; у promtail `429 Too Many Requests` | холодный старт: positions в emptyDir, promtail перечитывает все логи кластера и упирается в ingestion-лимит | поднять `limits_config.ingestion_rate_mb` (см. loki.yaml) или вынести positions на hostPath |
