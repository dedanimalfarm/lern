...
|---------|---------|--------------------|
| В Loki ВООБЩЕ нет логов | Promtail не нашёл targets: нет `HOSTNAME=spec.nodeName` или `__path__` | Проверьте: `kubectl -n lab logs ds/promtail \| grep -i target`. Убедитесь, что проброшен `HOSTNAME`. |
| `\| json` «не работает», метки пустые | CRI-префикс в теле строки (нет stage `cri`) | Посмотрите сырую строку в Loki. Если она начинается с даты/времени, а не с `{`, добавьте `pipeline_stages: [cri: {}]`. |
| Метрик-запрос падает HTTP 400 `pipeline error` | В выборку попали строки, которые не парсятся (в них есть `__error__`) | Добавьте `\| __error__=""` после парсера. |
| Запросы по `{namespace=...}` тормозят | Селектор слишком широкий — перебор всех чанков ns | Сузьте метками (`app`, `container`), потом используйте line-фильтры. |
| Loki потребляет много RAM/диска | Кардинальность меток или болтливое приложение | Проверьте объемы логов: `bytes_over_time by (app)`. Уберите уникальные идентификаторы (`request_id`, `user_id`) из меток (вынесите в тело лога). |
| Свежие логи приходят с задержкой в минуты; у promtail `429 Too Many Requests` | Холодный старт: positions в emptyDir, promtail перечитывает все логи кластера и упирается в ingestion-лимит | Поднимите `limits_config.ingestion_rate_mb` (см. `loki.yaml`) или вынесите positions на `hostPath`. |
| Часть логов имеет "мусорный префикс" | Данные записаны ДО применения `cri` stage | Данные в чанках неизменяемы. Подождите ретеншена или используйте regex-фильтр: `\|~ "^\\{"`. |
| Логи от `payment-api` не парсятся как JSON, хотя `cri` stage включен | Приложение иногда пишет обычные строки (не JSON) в stdout | Отфильтруйте строки с JSON до парсинга: `\|~ "^\\{" \| json` или обрабатывайте ошибку. |
...
Для автоматической проверки того, что все компоненты развёрнуты и логи собираются корректно, запустите скрипт проверки:

```bash
./verify/verify.sh
# Вывод об успешном прохождении:
# [OK] loki is ready
...
1. **Архитектура индексов:** Чем архитектура Loki принципиально отличается от Elasticsearch/OpenSearch с точки зрения индексации, и как это влияет на производительность и стоимость хранения логов?
2. **Promtail DaemonSet:** Почему агенты вроде Promtail всегда разворачиваются как DaemonSet, а не Deployment, и какие директории хоста (`hostPath`) им обязательно нужны для работы?
3. **Кардинальность:** Что такое кардинальность меток? Почему добавление `user_id` или `request_id` в метки Loki может быстро "убить" кластер, и как правильно искать по таким полям?
4. **Ошибки парсинга:** Почему обычный запрос для просмотра логов терпит невалидные строки, помечая их `__error__`, а метрический запрос с агрегацией по тем же логам (например, `rate()`) падает целиком? Как это обойти?
