...
## Ожидаемый результат
- П.1: серии по `level`; без `__error__=""` Loki отвечает 400 из-за строк, которые не
  разобрались как JSON, — вы объяснили, откуда они берутся.
- П.2: доля ошибок ~15 %; п.3: три квантили с разбивкой по `path`.
- П.5: выражение вида `sum(count_over_time({app="log-generator"} |= "ERROR" [5m])) > 30`
  — вы понимаете, чем `count_over_time` отличается от `rate` для алерта по количеству.
