...
<!-- NAV -->
**⬅ [18-centralized-logging](../18-centralized-logging/)** · [индекс курса](../../README.md) · [карта обучения](../../docs/02-learning-path.md) · **[20-batch-workflows](../20-batch-workflows/) ➡**
<!-- /NAV -->
...
1. Почему Kubernetes использует декларативную модель и Level-triggered подход в контроллерах вместо Edge-triggered (на основе событий)?
   ...
   Level-triggered значит «сверяемся с текущим состоянием», edge-triggered — «реагируем на
   событие». События теряются: рестарт контроллера, потерянный watch, пропущенный апдейт — и
   edge-triggered система навсегда расходится с реальностью. Level-triggered контроллер при
   каждом проходе читает фактическое состояние и сравнивает с желаемым, поэтому самовосстановим:
   перезапустили его — он просто увидит расхождение и починит. События в Kubernetes служат лишь
   триггером «пора сверить», а не источником истины.
...
2. На каком этапе HTTP-запроса API-сервер применяет правила `x-kubernetes-validations` (CEL)?
   ...
   На этапе **валидации объекта**, внутри apiserver: после authn/authz и мутирующих вебхуков,
   но до записи в etcd (и до валидирующих вебхуков). CEL-правила `x-kubernetes-validations`
   описаны в самой CRD, поэтому проверяются самим apiserver — отдельный вебхук не нужен, и
   ошибка возвращается клиенту сразу в ответе на запрос.
...
3. В чем ключевая разница между `ownerReferences` и `finalizers`? Приведите пример, когда нужно использовать каждый из механизмов.
   ...
   `ownerReferences` — про **уборку потомков**: сборщик мусора удаляет объекты, чей владелец
   исчез (Deployment → ReplicaSet → Pod; оператор → созданные им Deployment и Service).
   `finalizers` — про **уборку перед удалением самого объекта**: apiserver ставит
   `deletionTimestamp` и ждёт, пока контроллер выполнит внешние действия (удалит запись DNS,
   снимет диск в облаке, закроет подписку) и снимет свой finalizer. Первое — автоматика внутри
   кластера, второе — гарантия для внешних систем; зависший finalizer без живого контроллера
   даёт вечный `Terminating` (сценарий 02).
...
4. Какую угрозу безопасности предотвращает разделение ресурса на основной объект и `subresources.status`?
   ...
   Разделение `spec` и `status` на разные subresources позволяет выдавать разные права:
   пользователь может менять `spec` (желаемое), но не `status`, а контроллер — наоборот. Без
   этого любой, кто может редактировать объект, мог бы подделать статус («всё здорово») и
   обмануть зависимые системы и алерты; плюс обновление статуса не увеличивает
   `metadata.generation`, что не даёт контроллеру зациклиться на собственных записях.
...
5. Что означает термин "Идемпотентность" в контексте функции `Reconcile()`?
   ...
   Reconcile должен приводить систему к желаемому состоянию **и давать тот же результат при
   повторном вызове**: он вызывается многократно — по событиям, по таймеру, после рестарта.
   Практически это значит «создать, если нет; обновить, если отличается; ничего не делать, если
   совпадает» (`create or patch`), а не «создать» вслепую — иначе второй проход упадёт на
   `AlreadyExists` или наплодит дубликаты.
...
6. Что такое "Server-side pruning" и как он помогает бороться с опечатками в YAML?
   ...
   Server-side pruning — apiserver отбрасывает поля, не описанные в `openAPIV3Schema` CRD
   (включено по умолчанию для `apiextensions.k8s.io/v1`). Опечатка в имени поля (`replcias`)
   не сохраняется в объекте: после `apply` её просто нет в `kubectl get -o yaml`, и это сразу
   видно. Без pruning (старое поведение `x-kubernetes-preserve-unknown-fields: true`) мусорное
   поле молча сохранялось бы, а контроллер продолжал использовать значение по умолчанию.
