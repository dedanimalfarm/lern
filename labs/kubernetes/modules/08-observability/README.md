...
- [Часть 4: Troubleshooting и runbook](#часть-4-troubleshooting-и-runbook)
  ...
  - [4.4 SLI, SLO и error budget — на чём строится алерт](#44-sli-slo-и-error-budget--на-чём-строится-алерт)
...
<!-- NAV -->
**⬅ [07-config-security](../07-config-security/)** · [индекс курса](../../README.md) · [карта обучения](../../docs/02-learning-path.md) · **[09-helm-gitops](../09-helm-gitops/) ➡**
<!-- /NAV -->
...
```bash
# metrics-server нужен для `kubectl top` и HPA. На стенде Kubespray его ставит
# scripts/bootstrap/02-install-metrics-server.sh (с флагом --kubelet-insecure-tls).
...
```mermaid
flowchart TD
    C["Контейнер (PID 1)<br/>пишет stdout/stderr"] --> CRI["CRI (containerd)<br/>файл на ноде: /var/log/pods/NS_POD_UID/CONTAINER/N.log<br/>размер и ротация — политика kubelet (~10Mi × 5 файлов)"]
    K["kubectl logs"] --> API["kube-apiserver"] --> KL["kubelet целевой ноды"]
    KL -- "читает .log с диска" --> CRI
```
...
| Точка на шкале ресурса | Что это | Кто использует |
|---|---|---|
| `requests` | резерв, гарантированный минимум | scheduler — куда поставить Pod; QoS-класс |
| `usage` (сейчас) | фактическое потребление | `kubectl top`, HPA |
| `limits` | потолок | cgroup ядра: CPU троттлится, RAM → OOMKilled |

`usage` может быть **ниже** `requests` (зарезервировано зря, нода «простаивает») или **выше**
`requests` (burst в пределах `limits`) — это нормально и зависит от профиля нагрузки.
...
Линейность важна: нет смысла читать логи (`шаг 3`), если под в статусе `Pending` и даже не получил ноду (`шаг 2`), и нет смысла дебажить сеть (`шаг 5`), если приложение находится в `CrashLoopBackOff` (`шаг 1`).1`).

**Evicted — выселение kubelet'ом (не путать с OOMKilled):**

- **Порядок выселения:** при нехватке памяти или диска на ноде kubelet выселяет поды по
  QoS-классу — сначала `BestEffort`, потом `Burstable` сверх своих requests, последними
  `Guaranteed` — отдавая ресурсы критичным сервисам (модуль 02 — QoS, модуль 12).
...
### 4.4 SLI, SLO и error budget — на чём строится алерт

Сигналы из Частей 1–3 отвечают на вопрос «что сломалось». Чтобы решать, *когда* будить
человека, нужны три понятия из SRE-практики:

| Понятие | Что это | Пример для `obs-demo` |
|---|---|---|
| **SLI** (indicator) | измеримая доля «хороших» событий | доля HTTP-ответов не-5xx за окно; доля запросов быстрее 300 мс |
| **SLO** (objective) | цель по SLI на период | 99.5 % успешных ответов за 30 дней |
| **Error budget** | сколько «плохого» можно потратить, не нарушив SLO | 0.5 % от запросов месяца ≈ 3.6 часа полного простоя |

SLI считают из метрик, а не из логов (модуль 17 даёт `http_requests_total` у `metrics-app`):

```promql
# SLI доступности за 5 минут: доля не-5xx
sum(rate(http_requests_total{job="metrics-app",code!~"5.."}[5m]))
  / sum(rate(http_requests_total{job="metrics-app"}[5m]))

# Скорость сжигания бюджета (burn rate) при SLO 99.5 %: 1.0 = бюджет кончится ровно к концу окна
(1 - (sum(rate(http_requests_total{job="metrics-app",code!~"5.."}[1h]))
      / sum(rate(http_requests_total{job="metrics-app"}[1h])))) / (1 - 0.995)
```

Правило multi-window burn-rate (Google SRE Workbook): будить, если бюджет горит **в 14 раз
быстрее** нормы одновременно на окне 1 ч и 5 мин (быстрое сгорание), и заводить тикет при
6× на окнах 6 ч / 30 мин. Так алерт срабатывает на реальную деградацию и молчит на
единичных 5xx — в отличие от «`up == 0`» или «ошибок больше 10».

Почему это лучше порогов на симптомах: SLO привязывает алерт к тому, что видит пользователь,
а error budget превращает «стабильность» в число, которым можно торговаться с релизами
(бюджет сожжён — релизы стоп, чинить надёжность).
...
1. Чем `Events` отличаются от `Conditions`?
   ...
   `Events` — записи о **произошедшем** (что сделал контроллер, что не получилось у kubelet):
   короткоживущие, со временем и причиной. `Conditions` — поля `status.conditions` самого
   объекта, описывающие его **текущее** состояние (`Ready`, `Available`, `PodScheduled`) с
   `status`, `reason`, `lastTransitionTime`. События отвечают «что случилось», conditions —
   «как сейчас».
...
2. Почему events нельзя считать долговременным архивом логов аудита?
   ...
   События хранятся в etcd ограниченное время (по умолчанию около часа, флаг apiserver
   `--event-ttl`) и могут схлопываться. Это диагностический поток, а не журнал аудита: для
   истории нужен audit log apiserver или сбор событий во внешнюю систему
   (event-exporter → Loki/ELK). Поэтому при инциденте события снимают сразу.
...
3. Что означает `Available=True` у Deployment?
   ...
   `Available=True` означает, что доступно как минимум `replicas - maxUnavailable` подов,
   которые Ready дольше `minReadySeconds`. Это не «выкат завершён» (за это отвечает
   `Progressing` с reason `NewReplicaSetAvailable`) и не «все поды здоровы» — часть реплик может
   быть недоступна в пределах бюджета.
...
4. Зачем apiserver "схлопывает" повторяющиеся события (`count`, `lastTimestamp`)?
   ...
   Иначе один падающий под за минуты сгенерировал бы тысячи одинаковых записей и вытеснил бы
   из etcd всё остальное. Apiserver агрегирует повторы в одну запись со счётчиком `count`,
   `firstTimestamp` и `lastTimestamp` — в выводе это выглядит как `(x12 over 3m)`. Полезно и
   диагностически: видно частоту события, а не только факт.
...
5. В каких случаях обычный `kubectl logs` бесполезен и необходимо использовать флаг `--previous`?
   ...
   Когда контейнер уже перезапущен: `kubectl logs` читает **текущий** экземпляр, в котором
   причины падения нет. `--previous` отдаёт поток предыдущего, упавшего контейнера. Если
   контейнер жил доли секунды, логи могут не сохраниться вовсе — тогда причину берут из
...
6. Почему структурированные логи (JSON) упрощают расследование инцидентов в продакшене?
   ...
   JSON-строки парсятся без регулярных выражений: можно фильтровать по полям
   (`status=500`, `level=error`), считать метрики (`rate`, квантили `duration_ms`) и
   коррелировать с трейсами по `trace_id`. В модуле 18 на этом построены LogQL-парсеры: с
   `| json | status >= 500` запрос надёжнее и дешевле, чем grep по подстроке.
...
7. Как прочитать логи конкретного контейнера в многоконтейнерном поде? Опишите команду.
   ...
   `kubectl -n lab logs <pod> -c <container>`; для init-контейнера — то же с его именем.
   Полезные флаги: `--all-containers` (все контейнеры пода), `-l app=...` (по селектору),
   `-f` (следить), `--tail=N`, `--since=10m`, `--previous`.
...
8. Какая разница между метриками `Usage`, `Requests` и `Limits`? Откуда берется каждое из этих значений?
   ...
   `Requests` — заявка, берётся из манифеста; её использует scheduler и на неё смотрит квота.
   `Limits` — потолок из манифеста, его применяет cgroup (CPU троттлится, память → OOMKilled).
   `Usage` — фактическое потребление, его измеряет kubelet/cAdvisor и отдаёт metrics-server
   (`kubectl top`). Первые два — договор, третье — реальность; расхождение и есть предмет
   rightsizing.
...
9. Что даёт кластеру компонент `metrics-server` и чего он принципиально НЕ умеет по сравнению с Prometheus?
   ...
   metrics-server собирает текущие значения CPU/памяти подов и нод и отдаёт их через Metrics
   API — этого хватает для `kubectl top` и HPA. Он **не хранит историю** (только последние
   значения в памяти), не умеет произвольные метрики приложений, PromQL, алерты и агрегации.
   Всё это — Prometheus (модуль 17).
...
10. Почему `kubectl top nodes` может показывать загрузку CPU 15%, но при этом поды остаются в `Pending` из-за нехватки CPU (Insufficient cpu)?
   ...
   `kubectl top` показывает фактическое потребление, а scheduler считает по **requests**. Ноду
   можно «забронировать» под завязку подами, которые почти ничего не потребляют: сумма requests
   близка к allocatable, свободного места для нового пода нет, хотя реальный CPU простаивает.
   Проверяется `kubectl describe node` → блок `Allocated resources`.
...
11. Что такое `CrashLoopBackOff` и как работает алгоритм экспоненциальной задержки рестартов?
   ...
   Состояние, когда контейнер многократно падает и kubelet перезапускает его с нарастающей
   паузой: 10s, 20s, 40s … до потолка 5 минут (удваивается при каждом падении). Счётчик
   сбрасывается, если контейнер проработал достаточно долго. `CrashLoopBackOff` — это не
   причина, а следствие: причину ищут в `logs --previous` и `lastState.terminated`.
...
12. В чём фундаментальная разница между `OOMKilled` и `Evicted`? Кого убивает ядро, а кого выселяет kubelet?
   ...
   `OOMKilled` — процесс убит **ядром** за превышение `limits.memory` своей cgroup: страдает
   один контейнер, нода здорова, `exitCode: 137`, под перезапускается на месте. `Evicted` —
   **kubelet** выселил под целиком, потому что на ноде кончилась память/диск: под уходит в
   статус `Evicted`, а контроллер создаёт новый, возможно на другой ноде. Первое — про лимит
   пода, второе — про давление на ноду.
...
13. Опишите правильный порядок шагов диагностики (runbook) при падающем приложении.
   ...
   (1) Зафиксировать симптом: `get pods -o wide`, с каких пор (`AGE`, `RESTARTS`).
   (2) `describe pod` — conditions и события: не запланирован? не скачался образ? не прошла
   проба? (3) `logs` (и `--previous`) — что говорит приложение. (4) Ресурсы: `top pods/nodes`,
   `lastState` на OOM. (5) Сеть, если это сервис: `get endpoints`, DNS, политика.
   Линейность важна: нет смысла читать логи, если под ещё `Pending` и не получил ноду.
