...
> по-настоящему запускаются. Эмулятор нод (kwok) показывает объекты в API, но
> не выполняет контейнеры — `readinessProbe`, `logs` и `exec` на нём не
> отработают. Для прохождения берите kind/minikube/k3s. Все «ожидаемые выводы»
> ниже приведены для типового однонодового кластера (kube-dns на `10.96.0.10`,
> CNI выдаёт Pod IP из `10.42.0.0/16` либо `10.244.0.0/16`).
