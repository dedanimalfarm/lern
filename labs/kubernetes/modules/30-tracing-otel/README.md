...
<!-- NAV -->
**⬅ [29-pod-lifecycle-v2](../29-pod-lifecycle-v2/)** · [индекс курса](../../README.md) · [карта обучения](../../docs/02-learning-path.md) · **конец ➡**
<!-- /NAV -->
...
1. **Три столпа наблюдаемости:** Какие три сигнала мы собираем и на какой ключевой вопрос помогает ответить каждый из них при диагностике сложного инцидента?
   ...
   **Метрики** — «что и насколько плохо» (агрегаты: RPS, latency-квантили, ошибки; дёшевы,
   хранятся долго, по ним строят алерты). **Логи** — «что именно произошло» (дискретные события
   с деталями конкретного запроса). **Трейсы** — «где именно и почему долго» (путь одного
   запроса через сервисы с таймингом каждого шага). Инцидент обычно начинается с метрик
   (SLO горит), локализуется трейсами (какой хоп даёт задержку) и объясняется логами того
   сервиса.
...
2. **Путь спана:** Детально опишите полный путь спана от функции `requests.get()` в коде до кликабельной строки в дашборде Grafana. Перечислите все промежуточные звенья архитектуры и используемые протоколы.
   ...
   SDK в приложении создаёт спан и кладёт его в **BatchSpanProcessor** (буфер в памяти);
   экспортёр по OTLP (gRPC на 4317 или HTTP/protobuf на 4318) отправляет батч в **OTel
   Collector**. В коллекторе — pipeline: receiver → processors (batch, memory_limiter,
   resource/attributes) → exporter. Exporter шлёт в **Tempo** (тоже OTLP), тот пишет чанки в
   хранилище. **Grafana** через datasource Tempo делает запрос TraceQL по `trace_id` и рисует
   дерево спанов; ссылка «лог → трейс» строится derived field'ом по `trace_id` в логе.
...
3. **Роль Коллектора:** Назовите минимум 3 причины использовать промежуточный OTel Collector, даже если ваш бэкенд (Tempo) умеет прекрасно принимать OTLP данные напрямую от приложений?
   ...
   (1) **Отвязка приложения от бэкенда**: адрес и протокол меняются в коллекторе, а не в
   десятке сервисов. (2) **Обработка**: батчинг, ограничение памяти, sampling (хвостовой —
   только в коллекторе), удаление или маскирование чувствительных атрибутов, обогащение
   (k8s-метаданные). (3) **Надёжность и развязка**: буфер и ретраи при недоступности бэкенда,
   приложение не блокируется. Плюс единая точка для фан-аута в несколько систем (Tempo +
   метрики из спанов) и для смены вендора.
...
4. **Контекст Трейса:** Каким образом распределенный trace «склеивается» между двумя микросервисами, написанными на разных языках программирования? Какую именно информацию несет в себе HTTP-заголовок W3C Trace Context?
   ...
   Через заголовок **`traceparent`** (W3C Trace Context), который SDK на стороне клиента
   записывает в исходящий HTTP-запрос, а SDK на стороне сервера читает. Формат:
   `версия-trace_id(16 байт)-parent_span_id(8 байт)-trace_flags` (флаг sampled). Язык и
   фреймворк не важны: оба SDK реализуют один стандарт, поэтому серверный спан становится
   потомком клиентского. Рядом идёт `tracestate` — для вендорских данных.
...
5. **Интеграция с логами:** Почему для реализации бесшовного перехода «Лог → Трейс» в Python недостаточно просто установить мета-пакет `opentelemetry-distro`? Что еще нужно сделать на стороне приложения и на стороне Grafana Datasource?
   ...
   `opentelemetry-distro` даёт автоинструментирование (спаны для HTTP-клиента и сервера), но
   не связывает логи с трейсами. Нужно: на стороне приложения — включить инжекцию контекста в
   логи (`OTEL_PYTHON_LOG_CORRELATION=true` или `LoggingInstrumentor(set_logging_format=True)`),
   чтобы в каждой строке появились `trace_id`/`span_id`, и писать логи в формате, который
   разберёт LogQL (лучше JSON). На стороне Grafana — в datasource Loki настроить
   **derived field** с регуляркой по `trace_id` и ссылкой на datasource Tempo (а для обратного
   перехода — `tracesToLogsV2` в datasource Tempo).
...
6. **Troubleshooting:** Вы видите ошибку `frame header looked like an HTTP/1.1 header` в логах OTel Collector'а. О чем она говорит, почему она возникает и как ее исправить?
   ...
   Это классическое несовпадение транспорта OTLP: экспортёр говорит **gRPC**, а попал на
   **HTTP**-порт приёмника (или наоборот) — gRPC-сервер видит вместо HTTP/2-фрейма текстовый
   HTTP/1.1-заголовок. Причина обычно в `OTEL_EXPORTER_OTLP_ENDPOINT`, указывающем на 4318
   при `OTEL_EXPORTER_OTLP_PROTOCOL=grpc` (сценарий 02). Лечение — привести пару в
   соответствие: gRPC → `http://otel-collector:4317`, HTTP → `http://otel-collector:4318`
   (для HTTP — с путём `/v1/traces`).
