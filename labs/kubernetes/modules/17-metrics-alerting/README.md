...
<!-- NAV -->
**⬅ [16-secrets-management](../16-secrets-management/)** · [индекс курса](../../README.md) · [карта обучения](../../docs/02-learning-path.md) · **[18-centralized-logging](../18-centralized-logging/) ➡**
<!-- /NAV -->
...
1. Опишите фундаментальную разницу между Pull и Push моделями сбора метрик. Почему инженеры Kubernetes отдают безоговорочное предпочтение Pull-архитектуре?
   ...
   Pull: система мониторинга сама ходит за метриками на `/metrics` целей. Push: приложение
   отправляет метрики в коллектор. В Kubernetes поды эфемерны, адреса меняются каждые минуты —
   при pull это не проблема: Prometheus узнаёт цели из service discovery API кластера и всегда
   знает актуальный список. Плюс pull даёт бесплатную проверку живости (метрика `up`), не
   требует настройки адреса коллектора в каждом приложении и защищает от лавины данных при
   инциденте (частоту опроса контролирует Prometheus). Push остаётся для короткоживущих задач —
   для них есть Pushgateway.
...
2. Перечислите 4 основных типа метрик в формате Prometheus. Для чего применяется тип Histogram?
   ...
   **Counter** — только растёт (число запросов, ошибок); читается через `rate`/`increase`.
   **Gauge** — произвольно меняется (температура, число реплик, занятая память).
   **Histogram** — распределение по бакетам (`_bucket`, `_sum`, `_count`), даёт квантили,
   вычисляемые на сервере (`histogram_quantile`) и агрегируемые между инстансами — им меряют
   latency и размеры. **Summary** — квантили считает клиент; агрегировать между инстансами
   нельзя, поэтому в k8s-мире обычно берут Histogram.
...
3. Объясните разницу между Instant Vector и Range Vector. Почему функция `rate()` математически требует на вход именно Range Vector?
   ...
   Instant vector — одно значение на серию на данный момент (`http_requests_total`).
   Range vector — набор значений за окно (`http_requests_total[5m]`). `rate()` вычисляет
   скорость роста counter'а, то есть производную: для этого нужны минимум две точки внутри
   окна, плюс окно позволяет корректно обработать сброс счётчика при рестарте пода. Поэтому
   `rate(x)` без диапазона — синтаксическая ошибка.
...
4. Как конкретно `ServiceMonitor` находит ваше приложение? Опишите полную цепочку селекторов (от `ServiceMonitor` до Pod), включая обязательную роль метки релиза для Operator.
   ...
   `ServiceMonitor` отбирает **Service** по `spec.selector.matchLabels`, а `endpoints[].port`
   указывает **имя** порта этого Service (не номер). Через EndpointSlice сервиса Prometheus
   получает адреса Ready-подов и скрейпит их напрямую. Сам `ServiceMonitor` должен быть
   подхвачен оператором: у объекта `Prometheus` есть `serviceMonitorSelector`, и в
   kube-prometheus-stack он по умолчанию ищет метку `release: <имя helm-релиза>` (у нас
   `release: kps`). Нет метки — ServiceMonitor существует, но игнорируется (сценарий 01);
   не то имя порта — job создан, но таргетов нет (сценарий 02).
...
5. Что такое "кардинальность метрик"? Почему метка вида `request_path="/api/v1/users/12345/profile"` потенциально убьет ваш кластер по оперативной памяти?
   ...
   Кардинальность — число уникальных комбинаций меток, то есть число временных рядов. Каждый
   ряд — отдельная структура в памяти и на диске. Метка со значением из URL с идентификаторами
   даёт новый ряд на каждого пользователя и каждый путь: миллионы рядов, рост RAM в десятки раз
   и OOM Prometheus. Правильно — нормализовать: `request_path="/api/v1/users/:id/profile"`, а
   идентификаторы держать в логах или трейсах.
...
6. Зачем алерту нужно поле `for`? Опишите, как меняются статусы алерта (Inactive, Pending, Firing) от момента нормальной работы системы до момента отправки сообщения.
   ...
   `for` — сколько времени условие должно быть истинным непрерывно, прежде чем алерт станет
   боевым. Состояния: `Inactive` (выражение ложно) → при истинности `Pending` (идёт отсчёт
   `for`) → если условие продержалось всё это время, `Firing` и отправка в Alertmanager. Если
   условие стало ложным раньше — возврат в `Inactive` без уведомления. Так отсекаются
   кратковременные всплески и дребезг.
...
7. Какие важные задачи решает `Alertmanager`, которые принципиально не решает сам движок Prometheus?
   ...
   Prometheus умеет только вычислять правила и отправлять факт срабатывания. Alertmanager
   отвечает за доставку: **группировку** (один инцидент — одно сообщение вместо сотни),
   **маршрутизацию** по меткам (команда, severity, окружение), **подавление** (inhibition:
   при падении кластера не слать алерты про каждый под), **silences** (заглушить на время
   работ), дедупликацию от нескольких реплик Prometheus и интеграции (Slack, PagerDuty,
   Telegram, email) с повторами (`repeat_interval`).
