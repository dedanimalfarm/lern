...
<!-- NAV -->
**⬅ [19-crd-operators](../19-crd-operators/)** · [индекс курса](../../README.md) · [карта обучения](../../docs/02-learning-path.md) · **[21-stateful-systems](../21-stateful-systems/) ➡**
<!-- /NAV -->
...
1. Когда `parallelism < completions` и что это даёт в реальном production кластере? (Подсказка: ресурсное голодание БД)
   ...
   `completions` — сколько единиц работы надо выполнить всего, `parallelism` — сколько
   одновременно. Разрыв между ними — это дроссель: 1000 задач по 10 параллельно, чтобы не
   положить общую БД пулом соединений, не выесть квоту namespace и не выбить внешний API по
   rate limit. Job при этом всё равно доведёт работу до конца, просто волнами.
...
2. Почему в Job запрещено указывать `restartPolicy: Always`? Как это противоречит архитектурной сути объекта?
   ...
   Job по смыслу завершается: под должен когда-то перейти в `Succeeded`. `restartPolicy: Always`
   означает «перезапускать даже после успешного выхода» — под никогда не станет завершённым, и
   Job не сможет посчитать `completions`. Поэтому API запрещает такую комбинацию на уровне
   валидации.
...
3. Чем `restartPolicy: Never` отличается от `OnFailure` по числу создаваемых объектов Pod? Что лучше выбрать, если вам важно потом анализировать логи каждой неудачной попытки в Kibana/Elasticsearch?
   ...
   `Never` — на каждую попытку создаётся **новый Pod**, упавшие остаются в истории.
   `OnFailure` — kubelet перезапускает контейнер **в том же поде**, растёт `RESTARTS`, объектов
   меньше. Если логи каждой неудачной попытки должны уехать в Elasticsearch/Kibana, выбирайте
   `Never`: у каждой попытки свой под со своим потоком логов, который подхватит сборщик; при
   `OnFailure` логи прошлых попыток доступны только через `--previous` и теряются после
   следующего рестарта.
...
4. Перечислите 3 места (интерфейса), через которые конкретный контейнер внутри пода может узнать свой номер индекса в режиме Indexed.
   ...
   (1) Переменная окружения `JOB_COMPLETION_INDEX` (её подставляет kubelet).
   (2) Аннотация пода `batch.kubernetes.io/job-completion-index` — доступна через Downward API
   как файл или переменная. (3) Имя и hostname пода: `<job>-<index>-<hash>`, индекс можно
   извлечь из `hostname`. Первый способ — основной.
...
5. Для какой реальной задачи (приведите пример) Indexed Mode подходит лучше, чем паттерн work-queue с RabbitMQ? В чем минусы Indexed режима?
   ...
   Indexed хорош, когда множество работы известно заранее и делится детерминированно:
   «обработать 24 часовых партиции за сутки», «пересчитать шардов 0..N», распределённое
   обучение, где каждому воркеру нужен стабильный номер. Не нужен брокер, есть гарантия, что
   каждый индекс обработан ровно один раз. Минусы: перекос (один тяжёлый индекс держит весь
   Job), невозможность динамически добавить работу и необходимость заранее знать `completions`.
...
6. В чём огромное преимущество использования `podFailurePolicy: FailJob` (по коду выхода) перед банальным выставлением `backoffLimit: 0`?
   ...
   `backoffLimit: 0` бьёт по всем ошибкам одинаково: и по фатальной («нет такой таблицы»), и
   по временной (сеть моргнула) — ретраев нет вообще. `podFailurePolicy` разбирает **причину**:
   по коду выхода (`onExitCodes`: 1 — конфигурация, не повторять → `FailJob`; 42 — временная,
   `Ignore`, не считать попытку) и по условиям пода (`DisruptionTarget` — под выселен при
   обслуживании ноды, это не вина задачи). Итог: мгновенный отказ на невосстановимом и честные
   ретраи на транзиентном.
...
7. Что перебьёт — `backoffLimit`, который ещё не достигнут (например, упало 2 раза из 6), или истёкший глобальный `activeDeadlineSeconds`?
   ...
   `activeDeadlineSeconds` жёстче: это лимит на **весь** Job от старта, он действует поверх
   ретраев. По его истечении Job немедленно переходит в `Failed` с `reason: DeadlineExceeded`,
   активные поды убиваются, новые попытки не создаются — независимо от того, что `backoffLimit`
   ещё не исчерпан (сценарий 02).
...
8. Чем грозит кластеру отсутствие `ttlSecondsAfterFinished`, если в нем работает 1000 крон-джобов в день? Что именно переполнится?
   ...
   Завершённые Job и их поды остаются в etcd навсегда: тысяча Job в день — это десятки тысяч
   объектов Job + Pod за месяц. Переполняется etcd и раздувается кэш apiserver: `kubectl get
   pods` тормозит, watch'и становятся дороже, растёт потребление памяти control-plane.
   `ttlSecondsAfterFinished` удаляет Job (вместе с подами) через N секунд после завершения;
   для CronJob дополнительно есть `successfulJobsHistoryLimit`/`failedJobsHistoryLimit`.
...
9. Три значения `concurrencyPolicy`. В какой бизнес-ситуации вы бы выбрали `Replace`, отбросив старые данные ради новых?
   ...
   `Allow` — запускать параллельно; `Forbid` — пропустить новый запуск, пока идёт предыдущий;
   `Replace` — убить текущий и запустить новый. `Replace` уместен, когда ценен только свежий
   результат: пересчёт дашборда или кэша каждые 5 минут — если прошлый пересчёт завис, его
   данные уже устарели, и продолжать его бессмысленно. Для идемпотентных выгрузок и биллинга
   `Replace` опасен — он обрывает работу на полпути.
...
10. Как работает защита от шторма запусков (`startingDeadlineSeconds`)? Если кластер лежал неделю, попытается ли CronJob выполнить все пропущенные 168 часовых запусков сразу после включения?
   ...
   `startingDeadlineSeconds` задаёт окно, в течение которого пропущенный запуск ещё имеет смысл
   выполнить; всё, что старше, считается просроченным и просто пропускается. После недельного
   простоя CronJob **не** выполнит 168 запусков: контроллер учитывает только пропуски внутри
   дедлайна (и не более 100 — иначе выставит ошибку `Cannot determine if job needs to be
   started: too many missed start times`). Без `startingDeadlineSeconds` он выполнит ровно
   один — ближайший пропущенный.
