...
- [Архитектура и место батч-нагрузок в Kubernetes](#-)
  - [Жизненный цикл Job и Job Controller](#job--job-controller)
  - [Под капотом: OwnerReferences и Labels](#-ownerreferences--labels)
- [Часть 1: Job — parallelism и completions](#-1-job--parallelism--completions)
  - [Теория для изучения перед частью](#----)
  - [1.1 Запуск и наблюдение волн (Батч-процессинг)](#11-----)
  - [Паттерн: Рабочая очередь (Work Queue)](#--work-queue)
- [Часть 2: Indexed Job — партиционирование работы](#-2-indexed-job---)
  - [Теория для изучения перед частью](#----1)
  - [Архитектура статического шардирования (Indexed) vs Динамическая очередь (Work-Queue)](#-indexed-vs--work-queue)
  - [2.1 Запуск Indexed Job](#21--indexed-job)
- [Часть 3: Управление сбоями и временем](#-3----)
  - [Теория для изучения перед частью](#----2)
  - [3.1 podFailurePolicy: fail-fast по коду выхода](#31-podfailurepolicy-fail-fast---)
  - [3.2 podFailurePolicy: Ignore для Spot инстансов (DisruptionTarget)](#32-podfailurepolicy-ignore--spot--disruptiontarget)
  - [3.3 suspend и ttlSecondsAfterFinished (фрагменты)](#33-suspend--ttlsecondsafterfinished-)
- [Часть 4: CronJob — расписание и политики](#-4-cronjob----)
  - [Теория для изучения перед частью](#----3)
  - [Нюансы расписания и часовых поясов](#---)
  - [4.1 CronJob в действии](#41-cronjob--)
- [Часть 5: Troubleshooting — боевые инциденты](#-5-troubleshooting---)
  - [Теория: диагностика батч-задач](#---)
  - [Инцидент 1: Job не завершается — `BackoffLimitExceeded`](#-1-job----backofflimitexceeded)
  - [Инцидент 2: Зависший процесс и `activeDeadlineSeconds`](#-2---activedeadlineseconds)
  - [Инцидент 3: CronJob не запускает новые Job (эффект `Forbid`)](#-3-cronjob----job--forbid)
  - [Инцидент 4: Ошибка `ImagePullBackOff` и исчерпание лимитов](#-4--imagepullbackoff---)
  - [Инцидент 5: Контейнер падает с `OOMKilled`](#-5----oomkilled)
  - [Бонус: быстрая диагностика батч](#---1)
- [Проверка модуля](#-)
- [Финальная карта ресурсов модуля](#---)
- [Теоретические вопросы (итоговые)](#--)
  - [Блок 1: Job, parallelism, completions](#-1-job-parallelism-completions)
  - [Блок 2: Indexed](#-2-indexed)
  - [Блок 3: Сбои и время](#-3---)
  - [Блок 4: CronJob](#-4-cronjob)
