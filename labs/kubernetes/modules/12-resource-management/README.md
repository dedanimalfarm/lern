...
<!-- NAV -->
**⬅ [11-autoscaling](../11-autoscaling/)** · [индекс курса](../../README.md) · [карта обучения](../../docs/02-learning-path.md) · **[13-resilience](../13-resilience/) ➡**
<!-- /NAV -->
...
1. Что именно проверяет kube-scheduler при поиске ноды для нового пода: requests или limits?
   ...
   Только **requests**: scheduler ищет ноду, где сумма requests уже размещённых подов плюс
   requests нового не превышает allocatable. `limits` планировщик не интересуют — это
   ограничение времени выполнения. Поэтому нода может быть «забита» по requests при почти
   нулевом фактическом потреблении.
...
2. Какие механизмы ядра Linux (cgroups) отвечают за обеспечение limits для CPU и Памяти? В чем разница их поведения?
   ...
   CPU — `cpu.max` в cgroup v2 (квота на период): при превышении процесс **троттлится**, то
   есть ему просто не дают тактов, он замедляется, но живёт. Память — `memory.max`: страницы
   нельзя «замедлить», поэтому при превышении срабатывает OOM-killer ядра и процесс
   **убивается**. Отсюда правило: CPU-лимит делает медленно, memory-лимит убивает.
...
3. Что такое CPU Throttling и как он влияет на работу веб-сервиса?
   ...
   Троттлинг — принудительная пауза процесса, когда он выбрал CPU-квоту в текущем периоде
   (100 мс). Для веб-сервиса это всплески latency: запрос ждёт не ресурса, а конца периода.
   Видно по метрике `container_cpu_cfs_throttled_seconds_total`. Часто встречается у приложений
   с низким лимитом и многопоточным рантаймом (JVM, Go) — лечится повышением лимита или
   уменьшением числа потоков.
...
4. Что означает Exit Code 137 у контейнера?
   ...
   128 + 9 = SIGKILL: процесс убит извне. В контексте контейнера это почти всегда OOM-killer
   (превышен `limits.memory`) — подтверждается `reason: OOMKilled` в `lastState.terminated`.
   Реже — SIGKILL от kubelet при истечении `terminationGracePeriodSeconds`.

   </details>


### Блок 2: QoS и Eviction
5. Каким правилам должен соответствовать под, чтобы получить класс `Guaranteed`?

   <details><summary>Ответ</summary>

   У **каждого** контейнера пода (включая init) заданы и requests, и limits, и они равны по
   CPU и по памяти. Любое отклонение — отсутствие лимита, неравные значения, незаданные
   ресурсы у одного контейнера — переводит под в `Burstable`.

   </details>

6. В каком порядке Kubelet выселяет поды при исчерпании памяти на узле (Node-pressure)?

   <details><summary>Ответ</summary>

   Сначала `BestEffort`, затем `Burstable`, потребляющие сверх своих requests (тем раньше, чем
   больше превышение), последними — `Guaranteed`. Внутри класса учитывается
   `pod priority`. Идея: первыми уходят те, кто ничего не резервировал и на кого никто не
   рассчитывал.

   </details>

7. Какая разница между OOMKilled и Evicted? Кто их вызывает (ядро или Kubelet)?

   <details><summary>Ответ</summary>

   `OOMKilled` вызывает **ядро** (OOM-killer в cgroup контейнера) за превышение
   `limits.memory`: убит один контейнер, под перезапускается на месте, exit 137. `Evicted` —
   решение **kubelet** при давлении на ноду (память, диск, inodes): выселяется весь под, он
   получает статус `Evicted`, а контроллер создаёт новый, возможно на другой ноде.

   </details>


### Блок 3: PriorityClass и Preemption
8. В чем принципиальное отличие `PriorityClass` (preemption) от `QoS` (eviction)?

   <details><summary>Ответ</summary>

   `PriorityClass` работает **при планировании**: если для важного пода нет места, scheduler
   вытесняет (preempts) поды с меньшим приоритетом, чтобы освободить ресурсы. QoS работает
   **во время выполнения**: kubelet выселяет поды при нехватке ресурсов на ноде. Первое — про
   очередь за местом, второе — про выживание под давлением.

   </details>

9. Может ли планировщик вытеснить под с `PriorityClass` = 500, чтобы запустить под с `PriorityClass` = 100? А наоборот?

   <details><summary>Ответ</summary>

   Вытеснение идёт только сверху вниз: под с приоритетом 500 может вытеснить под с 100, но не
   наоборот. Планировщик ищет жертв **строго** с меньшим значением `priority`; если таких нет,
   высокоприоритетный под остаётся `Pending`.

   </details>

10. Что такое `system-node-critical` и зачем он нужен?

   <details><summary>Ответ</summary>

   Встроенный PriorityClass с очень высоким значением (2000001000) для компонентов, без которых
   нода нерабочая: CNI, kube-proxy, csi-драйверы (обычно DaemonSet'ы). Он гарантирует, что
   системные поды вытеснят пользовательские, а не наоборот, и не будут выселены при нехватке
   ресурсов. Рядом есть `system-cluster-critical` — для компонентов кластера (CoreDNS и т.п.).

   </details>


### Блок 4: Troubleshooting
11. Вы видите `Pending` под, а в events написано `FailedScheduling: Insufficient cpu`. Какой лимит нужно увеличить: кластера или пода?

   <details><summary>Ответ</summary>

   Зависит от того, адекватны ли requests пода. Если под просит больше, чем есть на любой
   ноде (например, 8 CPU при нодах по 2) — надо чинить **под**: уменьшить requests. Если
   requests разумны, а суммарно места нет — расширять **кластер** (добавить ноду или включить
   автоскейлер). Диагностика: `kubectl describe node` → `Allocated resources` против
   `kubectl get pod -o jsonpath='{..resources.requests}'`.

   </details>

12. Как LimitRange помогает администраторам избежать появления BestEffort подов в кластере?

   <details><summary>Ответ</summary>

   `LimitRange` с `defaultRequest`/`default` подставляет ресурсы каждому контейнеру, который
   их не указал, — и такой под автоматически становится `Burstable` вместо `BestEffort`.
   Дополнительно `min`/`max` не дают выйти за разумные рамки, а `maxLimitRequestRatio`
   ограничивает разрыв между requests и limits. Так администратор гарантирует, что в namespace
   нет подов «без договора» (см. сценарий 02).
