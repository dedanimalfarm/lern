...
## Ожидаемый результат
- П.1: успешный релиз останавливался на шаге с `pause`, `analysisrun` в статусе
  `Successful` с числом измерений = `count`.
- П.2: битый релиз стал `Degraded` примерно через 1–1.5 минуты; ваша арифметика по
  `initialDelay`, `interval`, `failureLimit` сходится с наблюдением ±15 с.
- П.3: `retry rollout` продолжает **тот же** rollout с рабочим образом, `undo` создал бы
  новую ревизию со старым образом.
- П.4: подобранные `count`/`failureLimit` записаны с объяснением цены — более долгий
  анализ и позже обнаруженная деградация.
