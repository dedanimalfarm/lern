...
При работе с Gateway API часто возникают следующие ситуации:

1. **Gateway не переходит в статус Programmed**
   - **Причина:** Контроллер Gateway (например, Envoy Gateway) не запущен, либо не найден указанный `GatewayClass`.
   - **Решение:** Проверьте поды в `envoy-gateway-system` и статус `GatewayClass` (`kubectl get gatewayclass`).
2. **HTTPRoute имеет статус Accepted: False**
   - **Причина:** В `HTTPRoute` неверно указан `parentRefs` (ошибка в имени Gateway или namespace), либо Gateway не разрешает привязку из других namespace.
   - **Решение:** Убедитесь, что имя в `parentRefs` совпадает с вашим Gateway. Проверьте манифест Gateway на предмет `listeners[].allowedRoutes`.
3. **HTTPRoute Accepted: True, но возвращается 404 (Not Found)**
   - **Причина:** Gateway принял маршрут, но ни один backend (Service) не найден или не имеет активных Endpoints.
   - **Решение:** Проверьте работоспособность подов сервиса (`kubectl get pods,svc,endpoints -n <namespace>`).
4. **Трафик не разделяется при указании весов (Traffic Splitting)**
   - **Причина:** Gateway API использует вероятностное распределение. На малом количестве запросов может казаться, что трафик идет только на один сервис. 
   - **Решение:** Сделайте больше запросов в цикле, либо проверьте правильность портов в `backendRefs`.
...
1. Почему Gateway API использует модель ролевого доступа и разделяет конфигурацию на GatewayClass, Gateway и HTTPRoute, в отличие от единого манифеста Ingress? Приведите пример из реальной практики, когда это решает проблему Ingress.
2. Как происходит связь между HTTPRoute и Gateway? Что произойдет, если HTTPRoute ссылается на Gateway, который находится в другом namespace, но не разрешает кросс-неймспейсовые маршруты?
3. Каков механизм разделения трафика (Traffic Splitting) в Gateway API "под капотом"? Зависит ли он от аннотаций, как это было в Ingress, и как Gateway controller узнает о весах?
4. В чем заключается функция `GatewayClass`? Можно ли в одном кластере одновременно использовать несколько разных `GatewayClass` (например, один для внутренних балансировщиков, а другой для внешних)? Обоснуйте ответ.
...
```
*(Этот скрипт удалит все созданные в рамках лаборатории ресурсы, включая Gateway Controller, CRD и namespace `lab-gateway`).*
