...
- [Стартовая проверка](#-)
- [Часть 1: topologySpreadConstraints](#-1-topologyspreadconstraints)
  - [Теория для изучения перед частью](#----)
  - [1.1 Равномерное распределение](#11--)
- [Часть 2: podAntiAffinity](#-2-podantiaffinity)
  - [Теория для изучения перед частью](#----)
  - [2.1 Реплики на разных нодах](#21----)
- [Часть 3: PodDisruptionBudget](#-3-poddisruptionbudget)
  - [Теория для изучения перед частью](#----)
  - [3.1 PDB и drain](#31-pdb--drain)
  - [3.2 Обслуживание ноды: cordon → drain → uncordon](#32---cordon--drain--uncordon)
- [Часть 4: Troubleshooting — частые проблемы](#часть-4-troubleshooting--частые-проблемы)
  - [Инцидент 1: реплика `Pending` из-за required anti-affinity](#инцидент-1-реплика-pending-из-за-required-anti-affinity)
  - [Инцидент 2: PDB делает ноду недренируемой](#инцидент-2-pdb-делает-ноду-недренируемой)
...
Если вы изменяете настройки `topologySpreadConstraints` на уже запущенном Deployment, старые поды не будут перераспределены автоматически. Балансировка происходит только в момент планирования новых подов. Решение: сделать rollout restart или удалить поды вручную, чтобы ReplicaSet пересоздал их с учётом новых правил.
...
2. **Анатомия Eviction API и влияние PDB**: Опишите по шагам процесс, происходящий "под капотом" при выполнении команды `kubectl drain`. Какую роль в этом процессе играет Eviction API и как именно `PodDisruptionBudget` блокирует или разрешает выселение пода, основываясь на показателе `ALLOWED DISRUPTIONS`?
3. **Краевые случаи PDB и HPA**: Представьте ситуацию: Deployment масштабируется вниз (scale down) с помощью HPA (Horizontal Pod Autoscaler) одновременно с попыткой дренирования ноды администратором. Как это может привести к зависанию процесса `drain`, если настроен `minAvailable` в процентах или абсолютных значениях? Как избежать этой проблемы?
4. **Влияние Taints на расчет доменов**: Зачем при использовании `topologySpreadConstraints` в кластерах с заtaint-ованными Control Plane нодами необходимо указывать `nodeTaintsPolicy: Honor`? Что произойдет с подом при `whenUnsatisfiable: DoNotSchedule` и дефолтном `Ignore`, если недоступные из-за taints ноды будут учитываться как домены с нулем запущенных реплик?
...
Для полной очистки кластера от созданных ресурсов, пространств имен и возвращения нод в исходное состояние (снятие `cordon`), запустите подготовленный скрипт:
