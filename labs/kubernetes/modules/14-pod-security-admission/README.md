...
<!-- NAV -->
**⬅ [13-resilience](../13-resilience/)** · [индекс курса](../../README.md) · [карта обучения](../../docs/02-learning-path.md) · **[15-network-policy-enforced](../15-network-policy-enforced/) ➡**
<!-- /NAV -->
...
1. **Контейнерная изоляция:** Почему запуск процесса в Docker-контейнере от имени `root` представляет угрозу безопасности для всего физического сервера (ноды)?
   ...
   Контейнер — это процесс на хосте, изолированный namespace'ами и cgroups, а не виртуальная
   машина. UID 0 внутри контейнера по умолчанию совпадает с root на ноде (без user namespaces),
   поэтому любая ошибка изоляции — смонтированный `hostPath`, лишняя capability, доступ к
   `/var/run/docker.sock`, уязвимость рантайма — превращается в root на ноде и компрометацию
   всех подов на ней.
...
2. **PSS:** Назовите три официальных стандарта (профиля) Pod Security. Каким типам рабочих нагрузок соответствует каждый из них?
   ...
   `privileged` — без ограничений (системные компоненты, CNI, агенты); `baseline` — запрещает
   явно опасное: hostNetwork/hostPID/hostIPC, hostPath, privileged, добавление большинства
   capabilities (обычные приложения, минимальная миграция); `restricted` — всё из baseline плюс
   non-root, drop ALL, seccomp `RuntimeDefault`, запрет privilege escalation (прод-нагрузки и
   мульти-тенант namespace).
...
3. **PSA:** На каком этапе обработки запроса в Kubernetes API происходит отклонение пода механизмом PSA? Почему под даже не записывается в etcd?
   ...
   На этапе **validating admission**, после аутентификации, авторизации и мутирующих вебхуков,
   но до записи в etcd. PSA встроен в apiserver, отвечает «да/нет» и ничего не меняет.
   Поэтому отклонённого пода нет ни в `kubectl get pods`, ни в истории — ошибка возвращается
   прямо клиенту (`kubectl apply`), а при создании через контроллер оседает в событиях
   ReplicaSet.
...
4. **PSA vs PSP:** Назовите две главные причины, по которым разработчики Kubernetes решили полностью удалить функционал PodSecurityPolicy (PSP) в версии 1.25.
   ...
   (1) Привязка через RBAC: политика применялась к ServiceAccount пода, а при нескольких
   подходящих PSP выбиралась «первая по алфавиту» — поведение было непредсказуемым и почти
   недиагностируемым. (2) PSP **мутировал** объекты (подставлял `runAsUser`), из-за чего
   итоговый под отличался от отправленного, а миграция и отладка превращались в гадание.
   PSA устроен наоборот: метка на namespace, детерминированный выбор, только валидация.
...
5. **Миграция:** Почему стратегия внедрения безопасности через метки `warn` и `audit` является обязательной для production кластеров, прежде чем включать `enforce`?
   ...
   Потому что `enforce` — это одномоментный отказ в создании подов: включив его на живом
   namespace, вы роняете следующий же rollout. `warn` возвращает предупреждение клиенту, а
   `audit` пишет отметку в audit log — оба режима позволяют собрать список нарушений
   существующих нагрузок, починить их и только потом включать enforce. Все три метки можно
   держать одновременно с разными профилями (enforce: baseline + warn/audit: restricted).
...
6. **VAP:** Чем архитектурно `ValidatingAdmissionPolicy` (CEL) превосходит внешние `Validating Webhooks` с точки зрения надежности и задержек?
   ...
   VAP исполняется **внутри** apiserver: нет сетевого вызова, нет сертификатов вебхука, нет
   внешнего деплоя, который может упасть. Внешний вебхук добавляет задержку к каждому запросу и
   становится единой точкой отказа: при `failurePolicy: Fail` и недоступном вебхуке кластер
   перестаёт принимать объекты, при `Ignore` — молча теряет контроль. CEL-выражения ещё и
   дешевле по latency (микросекунды против миллисекунд).
...
7. **Конвейер Admission:** Объясните парадокс: почему под, который в исходном YAML файле выглядел полностью безопасным, может быть заблокирован PSA с ошибкой о наличии опасных capabilities? (Подсказка: Mutating Webhooks).
   ...
   Потому что PSA валидирует объект **после** мутирующих вебхуков. Ваш YAML был чистым, но
   service mesh или агент безопасности добавил sidecar с `NET_ADMIN`/`privileged` — и на
   валидацию пришёл уже изменённый под. Диагностика: смотреть итоговый объект
   (`kubectl get pod -o yaml`) и список мутирующих вебхуков
...
8. **Policy Engines:** В каких трех сценариях возможностей VAP вам гарантированно не хватит, и вы будете вынуждены устанавливать Kyverno или OPA Gatekeeper?
   ...
   (1) Нужна **мутация** — подставить недостающие поля (securityContext, labels, sidecar):
   VAP только валидирует (MutatingAdmissionPolicy ещё не везде доступна). (2) Нужна
   **генерация** сопутствующих объектов — создать default-deny NetworkPolicy и квоту на каждый
   новый namespace. (3) Нужны сложные политики со внешним контекстом и отчётностью: проверка
   подписи образов (`verifyImages`), обращение к данным других ресурсов, готовые
   policy-report'ы для комплаенса.
