...
<!-- NAV -->
**⬅ [08-observability](../08-observability/)** · [индекс курса](../../README.md) · [карта обучения](../../docs/02-learning-path.md) · **[10-kubeadm-admin](../10-kubeadm-admin/) ➡**
<!-- /NAV -->
...
1. Обязательная структура Helm chart и роль элементов (`Chart.yaml`, `values.yaml`, `templates/`).
   ...
   `Chart.yaml` — метаданные чарта (`name`, `version` — версия чарта, `appVersion` — версия
   приложения, зависимости); `values.yaml` — значения по умолчанию и, по сути, публичный
   интерфейс чарта; `templates/` — Go-шаблоны манифестов плюс `_helpers.tpl` с именованными
   шаблонами и `NOTES.txt`. Дополнительно: `charts/` (вендоренные зависимости), `crds/`
   (устанавливаются до остальных ресурсов и не обновляются автоматически).
...
2. Как templating связывает `values.yaml` и манифесты? Что делают `toYaml`, `{{- if }}`, `{{- with }}`?
   ...
   Helm рендерит шаблоны, подставляя дерево `.Values` (плюс `.Release`, `.Chart`,
   `.Capabilities`). `toYaml` печатает структуру как YAML (обычно с `nindent` для отступов),
   `{{- if }}` включает блок по условию, `{{- with }}` входит в под-объект и меняет область
   видимости `.`. Дефис в `{{-` съедает предшествующий пробельный вывод — без него в манифесте
   останутся пустые строки, и YAML может сломаться.
...
3. Приоритет values: что перебьёт что между `values.yaml`, `-f file`, `--set`? Зачем это нужно?
   ...
   Приоритет по возрастанию: `values.yaml` чарта → `-f my-values.yaml` (несколько файлов —
   каждый следующий перекрывает предыдущий) → `--set` → `--set-string`/`--set-file`. Это даёт
   слоистую конфигурацию: базовые значения в чарте, окружение в файле, разовое переопределение
   в команде (например, тег образа из CI).
...
4. В чём суть GitOps и что такое drift? Как `selfHeal` и `prune` его устраняют?
   ...
   GitOps: желаемое состояние описано в git, а контроллер в кластере непрерывно приводит
   кластер к нему. Drift — расхождение живого состояния с git (кто-то сделал `kubectl edit`,
   оператор дописал поле). `selfHeal` возвращает изменённые объекты к версии из git,
   `prune` удаляет объекты, которых в git больше нет.
...
5. Назовите 4 ключевых компонента Argo CD (repo-server, application-controller и др.) и роль каждого в sync-loop.
   ...
   `repo-server` — клонирует репозиторий и рендерит манифесты (`helm template`,
   `kustomize build`); `application-controller` — сравнивает desired и live, считает
   sync/health, применяет изменения; `redis` — кэш рендеринга и diff; `server` — API и UI
   (плюс `dex` для SSO). В модуле 25 добавляется `applicationset-controller`, генерирующий
   Application'ы.
...
6. Чем **sync**-статус отличается от **health**-статуса? Приведите пример, когда может быть `Synced` + `Progressing` одновременно и почему это не ошибка.
   ...
   Sync отвечает на вопрос «совпадает ли кластер с git», health — «в порядке ли сами
   ресурсы». `Synced + Progressing` — нормальная ситуация сразу после применения: манифесты уже
   совпадают с git, но Deployment ещё разворачивает поды. Тревожно, когда `Synced + Degraded`
   (git применён, но ресурс сломан) или `OutOfSync` надолго.
...
7. Как `Application` и `AppProject` описывают и ограничивают деплой? Почему нельзя все деплоить в дефолтный `default` AppProject?
   ...
   `Application` описывает конкретный деплой: откуда (`source`: repoURL, path,
   targetRevision), куда (`destination`: сервер и namespace) и как (`syncPolicy`).
   `AppProject` — рамка: список разрешённых репозиториев, назначений и типов ресурсов, плюс
   RBAC. Дефолтный `default` разрешает всё и всем — это отсутствие границы: любая опечатка или
   компрометация одного приложения даёт доступ ко всему кластеру.
...
8. Какие существуют альтернативы хранению зашифрованных секретов (SOPS) в Git-репозитории?
   ...
   SOPS+age/KMS — один из вариантов. Альтернативы: **Sealed Secrets** (шифртекст в git,
   расшифровать может только контроллер этого кластера), **External Secrets Operator**
   (в git лежит ссылка, значение — во внешнем менеджере), **Vault Secrets Operator** и
   CSI-драйвер секретов (значение приезжает из Vault, в том числе динамическое, с TTL).
   Подробно — модуль 16.
...
9. Что произойдет, если мы удалим `Application` из кластера с ключом `--cascade=true` (по умолчанию)?
   ...
   Каскадное удаление: Argo CD удаляет не только объект `Application`, но и все развёрнутые им
   ресурсы (это делает finalizer `resources-finalizer.argocd.argoproj.io`). Если нужно убрать
   только запись об управлении, оставив ресурсы, — удаляют с
   `--cascade=false`/`kubectl patch` со снятием finalizer. Важная деталь эксплуатации: при
   недоступном prune (например, удалён AppProject) delete зависает именно на этом finalizer.
