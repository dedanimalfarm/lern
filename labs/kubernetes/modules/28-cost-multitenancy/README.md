...
- [Теоретические вопросы (итоговые)](#теоретические-вопросы-итоговые)
  - [Блок 1: Модели изоляции](#блок-1-модели-изоляции)
  - [Блок 2: HNC](#блок-2-hnc)
  - [Блок 3: vcluster](#блок-3-vcluster)
  - [Блок 4: FinOps](#блок-4-finops)
...
| Модель (слева направо — сильнее изоляция, выше стоимость и overhead) | Изоляция | Где в курсе |
|---|---|---|
| Общий Namespace, метки/RBAC | слабая | м01–м07 |
| Namespace на тенанта + Quota/NetPol/PSA | средняя | Project E |
| Иерархия Namespace (HNC), деревья политик | средняя+ | Часть 2 |
| vcluster — свой API-сервер | сильная | Часть 3 |
| Отдельный кластер | полная | вне курса |
...
```mermaid
flowchart LR
    subgraph host["Host-кластер: стенд Kubespray, ns lab"]
        sts["StatefulSet my-vcluster-0<br/>kube-apiserver + controllers тенанта<br/>свой API-мир, свои CRD"]
        pvc["PVC data (SQLite/kine)<br/>состояние виртуального кластера"]
        shadow["nginx-x-default-x-my-vcluster<br/>физическая «тень» пода тенанта"]
        sts --- pvc
        sts -- "syncer создаёт теневые поды в ns lab" --> shadow
    end
```
...
### Блок 1: Модели изоляции

1. Расставьте по возрастанию изоляции: общий namespace с метками, namespace на тенанта с квотой и политиками, иерархия HNC, vcluster, отдельный кластер. Что именно усиливается на каждом шаге и чем за это платят?

   <details><summary>Ответ</summary>

   Общий namespace (метки + RBAC) → namespace на тенанта (+ ResourceQuota, LimitRange,
   NetworkPolicy, PSA) → иерархия HNC (политики наследуются деревом) → vcluster (свой
   API-сервер, свои CRD и namespace у тенанта) → отдельный кластер (полная изоляция
   control-plane и нод). С каждым шагом растёт изоляция управляющего слоя: сначала тенанты
   делят один API и видят общие объекты, потом получают собственный API-сервер, потом
   собственное железо. Платят накладными расходами и стоимостью: vcluster — это ещё один
   apiserver на тенанта, отдельный кластер — полный дубль control-plane и эксплуатации.

   </details>

2. Почему namespace сам по себе не является границей безопасности? Назовите три вещи, которые тенант всё равно видит или на которые влияет.

   <details><summary>Ответ</summary>

   Namespace изолирует **имена и права**, но не ядро и не кластер: (1) тенант видит и использует
   общие cluster-scoped ресурсы — ноды, StorageClass, CRD, PriorityClass; (2) поды разных
   тенантов работают на одних нодах и делят ядро (побег из контейнера или «шумный сосед» бьёт
   по всем); (3) без NetworkPolicy сеть плоская — поды из разных namespace свободно ходят друг
   к другу. Отсюда и нужны PSA, квоты, политики и — в пределе — vcluster.
...
3. Чем `HierarchyConfiguration` отличается от `SubnamespaceAnchor` и когда применяют каждый?

   <details><summary>Ответ</summary>

   `HierarchyConfiguration` (объект `hierarchy` в **дочернем** namespace с `spec.parent`)
   назначает родителя **уже существующему** namespace. `SubnamespaceAnchor` создаётся в
   **родительском** namespace, и HNC по нему сам создаёт дочерний namespace (и удаляет его
   вместе с якорем). Первый — для интеграции существующих namespace, второй — штатный способ
   заводить новые: тенант получает право создавать якоря, а не namespace.

   </details>

4. Что именно HNC распространяет от родителя к детям и что произойдёт с этими объектами, если изменить их в дочернем namespace?

   <details><summary>Ответ</summary>

   Распространяются объекты типов, помеченных в `HNCConfiguration` как `Propagate` — по
   умолчанию RoleBinding и Role, часто добавляют NetworkPolicy, LimitRange, ResourceQuota,
   Secret. Копии в дочернем namespace помечены аннотацией `hnc.x-k8s.io/inherited-from`; править
   или удалять их бессмысленно — HNC восстановит их из родителя. Менять надо оригинал.

   </details>

5. Почему вебхук HNC запрещает циклы в иерархии и что сломалось бы, будь они разрешены?

   <details><summary>Ответ</summary>

   Потому что распространение идёт строго вниз по дереву: при цикле («A родитель B, B родитель
   A») объекты копировались бы по кругу бесконечно, а вопросы «чьи политики главнее» и «какая
   квота действует» не имели бы ответа. Вебхук `hierarchyconfigurations.hnc.x-k8s.io`
   отклоняет такую конфигурацию на admission с сообщением `cycle` (сценарий 02).
...
6. Где физически работают поды тенанта, созданные внутри vcluster, и под какими именами они видны в host-кластере?

   <details><summary>Ответ</summary>

   В host-кластере, в том же namespace, где живёт сам vcluster. Syncer переписывает имена:
   под `nginx` из namespace `default` виртуального кластера `my-vcluster` появляется на хосте
   как `nginx-x-default-x-my-vcluster`. Внутри vcluster тенант видит привычное имя `nginx` —
   трансляция скрыта.

   </details>

7. Есть ли у vcluster собственные ноды и кто в итоге планирует поды тенанта?

   <details><summary>Ответ</summary>

   Своих нод нет. Внутри vcluster ноды — это «псевдообъекты», синхронизированные с хоста;
   реальное планирование делает **планировщик host-кластера**, потому что физически поды
   создаются именно там. Поэтому taints, квоты и ёмкость хоста действуют на тенанта, даже
   если внутри vcluster он полный админ.

   </details>

8. Почему квоты и политики host-namespace остаются последней линией защиты, даже если тенант — админ в своём vcluster?

   <details><summary>Ответ</summary>

   Потому что всё, что тенант создаёт внутри, материализуется подами в host-namespace и
   подпадает под его ResourceQuota, LimitRange, PSA и NetworkPolicy. Внутри своего API тенант
   может выключить любые ограничения — на хосте они останутся. Если он запросит больше остатка
   квоты, под просто не будет создан на хосте, а внутри vcluster зависнет в `Pending`
   (сценарий 01) — это и есть граница, которую тенант не может подвинуть.
...
9. Почему стоимость namespace считают от `requests`, а не от фактического потребления? В каком случае справедливее считать по usage?

   <details><summary>Ответ</summary>

   Потому что деньги тратятся на **забронированную** ёмкость: ядро, отданное под `requests`
   одного namespace, нельзя продать другому, даже если оно простаивает. Считать по usage
   справедливо там, где ресурсы действительно переиспользуются и не резервируются — burstable-
   нагрузки без гарантий, спотовые пулы, serverless-модель оплаты (платим за фактическое
   выполнение).

   </details>

10. Что такое overprovisioning и как его измерить одним запросом PromQL? Чем он лечится?

   <details><summary>Ответ</summary>

   Overprovisioning — разрыв между забронированным и потребляемым: команда просит 1 CPU, а
   использует 0.1. Измеряется отношением usage к requests, например:
   `sum by (namespace) (rate(container_cpu_usage_seconds_total{container!=""}[10m]))
   / sum by (namespace) (kube_pod_container_resource_requests{resource="cpu"})`.
   Лечится rightsizing'ом (правкой requests по факту, вручную или через рекомендации VPA);
   на стенде утилизация ns `lab` обычно около 20–25 %.
