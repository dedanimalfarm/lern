...
## Ожидаемый результат
- Таблица namespace → $/час из третьего запроса; сумма по всем namespace ≈ цена брони
  всего стенда.
- Для `lab` посчитана утилизация CPU (usage / requests) и названы поды с наибольшим
  разрывом.
- Вы объяснили, почему showback считают от requests, а не от usage, и в каком случае
  тенанту всё же выставляют счёт по фактическому потреблению.
