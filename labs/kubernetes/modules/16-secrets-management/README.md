...
1. В чём заключаются фундаментальные отличия и риски использования обычных `Secret` в Kubernetes (даже с base64) по сравнению с включённым `encryption-at-rest` в etcd?
   ...
   Обычный Secret — это base64 (кодирование, не шифрование) в etcd: любой, кто получил доступ к
   файлам/бэкапам etcd или к API с правом `get secrets`, читает значение открытым текстом.
   `encryption-at-rest` (`EncryptionConfiguration` у apiserver) шифрует данные перед записью в
   etcd — в дампе видно `k8s:enc:aescbc:v1:` вместо содержимого. Это защищает от утечки
   носителя/бэкапа, но **не** от того, кто имеет права читать Secret через API: apiserver
   расшифрует его сам.
...
2. Какие провайдеры `EncryptionConfiguration` считаются самыми безопасными для production (kms vs aescbc)?
   ...
   `kms` (v2) — ключ живёт во внешнем KMS/HSM (облачный KMS, Vault Transit), apiserver
   оперирует DEK'ами, ключ можно ротировать и отзывать, он не лежит на диске control-plane.
   `aescbc`/`secretbox` держат ключ в файле рядом с apiserver: компрометация ноды = компрометация
   всех секретов. Для прода рекомендуется kms v2; `aescbc` — приемлемый минимум, `identity`
   (по умолчанию) — отсутствие шифрования.
...
3. Архитектура Sealed Secrets подразумевает асимметричное шифрование. Каким образом `kubeseal` понимает, каким публичным ключом шифровать данные, и как предотвращается расшифровка секрета на другом кластере?
   ...
   Контроллер Sealed Secrets генерирует пару ключей и публикует публичный; `kubeseal` забирает
   его прямо из кластера (`--controller-namespace`) или из сохранённого файла (`--cert`).
   Шифрование привязано к паре, и в шифртекст входят имя и namespace секрета — расшифровать
   может только контроллер с соответствующим приватным ключом. На другом кластере тот же файл
   даст `no key could decrypt secret`; это by design — файл можно спокойно держать в публичном git.
...
4. Почему критически важно создавать резервные копии ключей Sealed Secrets контроллера?
   ...
   Приватный ключ контроллера — единственный способ расшифровать все SealedSecret'ы. Потеряли
   кластер вместе с ключом (пересоздали, потеряли etcd) — все зашифрованные секреты в git
   превращаются в мусор, их придётся генерировать и запечатывать заново. Ключи лежат в Secret'ах
   `kube-system` с меткой `sealedsecrets.bitnami.com/sealed-secrets-key`; их бэкапят и хранят
   вне кластера.
...
5. В чем разница между `SecretStore` и `ExternalSecret` в ESO?
   ...
   `SecretStore` (или `ClusterSecretStore`) описывает **источник**: провайдер (Vault, AWS SM,
   GCP SM, fake), адрес и способ аутентификации. `ExternalSecret` описывает **что и куда**:
   какие ключи из источника забрать, в какой k8s Secret положить, под какими именами и как
   часто обновлять (`refreshInterval`). Первый — про подключение, второй — про синхронизацию.
...
6. Что находится в Git-репозитории при использовании External Secrets Operator, а что во внешнем хранилище?
   ...
   В git — только `ExternalSecret` (и `SecretStore`): ссылки на ключи и параметры синка, без
   самих значений. Значения живут во внешнем менеджере, а в кластере появляются как обычный
   Secret, созданный оператором. Поэтому репозиторий можно ревьюить и хранить публично, а
   ротация выполняется в менеджере без коммитов.
...
7. Объясните механизм аутентификации Vault Secrets Operator (VSO) в кластере Vault. Какую роль в этом играет Kubernetes ServiceAccount и права `system:auth-delegator`?
   ...
   VSO берёт токен смонтированного ServiceAccount (`vso-auth`) и отправляет его в Vault через
   auth-метод `kubernetes`. Vault проверяет токен, обращаясь к **TokenReview API** кластера —
   для этого его собственный ServiceAccount должен иметь ClusterRole `system:auth-delegator`.
   Убедившись в подлинности, Vault сопоставляет SA и namespace с ролью
   (`bound_service_account_names`/`namespaces`) и выдаёт токен Vault с политикой этой роли.
   Статических паролей в цепочке нет — доверие строится на identity кластера.
...
8. Динамические секреты Vault создают новые учетные данные on-demand с заданным TTL. Каким образом обеспечивается автоматическая ротация этих секретов в Kubernetes Secret без ручного вмешательства, и как приложение узнаёт об их обновлении?
   ...
   `VaultDynamicSecret` знает TTL выданных кредов: оператор заранее (до истечения) запрашивает
   новые, записывает их в тот же k8s Secret и обновляет его. Приложение узнаёт об изменении
   по-разному: если Secret смонтирован **томом**, kubelet обновляет файлы, и приложение может
   отследить это через inotify и перечитать; если Secret проброшен в **env**, обновление не
   дойдёт до процесса — нужен рестарт (`rolloutRestartTargets` в спецификации VSO).
