...
1. В чём принципиальное различие между обычным `initContainer` (без restartPolicy) и native sidecar (с `restartPolicy: Always`) с точки зрения блокировки запуска основных контейнеров?
   ...
   Обычный init-контейнер **блокирует**: следующий init и основные контейнеры не стартуют, пока
   он не завершится с кодом 0. Native sidecar (init с `restartPolicy: Always`) не должен
   завершаться: kubelet ждёт лишь того, что он **Started** (и прошёл startupProbe, если задана),
   и сразу переходит к следующим контейнерам. Так прокси или агент гарантированно поднят до
   приложения, но не мешает ему запуститься.
...
2. Почему размещение sidecar-контейнера в массиве `containers[]` ломает логику работы ресурсов типа `Job`?
   ...
   Job считается выполненным, когда завершатся **все** контейнеры из `containers[]`. Вечный
   sidecar (лог-шиппер, istio-proxy) никогда не выходит, поэтому под остаётся в статусе
   `1/2 NotReady`, а Job — `Active` навсегда, пока не сработает `activeDeadlineSeconds`.
   Native sidecar эту проблему снимает: завершение отслеживается только по `containers[]`, а
   sidecar гасится автоматически (сценарий 01).
...
3. В каком порядке kubelet отправляет сигнал SIGTERM контейнерам при остановке пода с native sidecar? Почему выбран именно такой порядок?
   ...
   Сначала SIGTERM получают основные контейнеры (`containers[]`), и только после их завершения —
   native sidecar'ы, в порядке, обратном запуску. Логика та же, что при старте, но наоборот:
   приложение должно успеть дослать метрики, логи и закрыть соединения через прокси — значит,
   вспомогательные контейнеры обязаны пережить его.
...
4. Каким образом механизм scheduling gates помогает экономить ресурсы нод и IP-адреса кластера по сравнению с традиционным подходом (busy-wait циклы в initContainers)?
   ...
   Под с gate вообще не попадает в очередь планировщика: он не занимает место на ноде, ему не
   выделяется IP (сеть настраивается при запуске на ноде), не тратятся ресурсы на запущенный
   init-контейнер. Традиционный подход — под уже запущен на ноде, и его init крутит цикл
   ожидания, занимая CPU, память, IP и слот пода, иногда часами.
...
5. Может ли kube-scheduler самостоятельно снять gate с пода, если на ноде появилось достаточно ресурсов? Почему?
   ...
   Нет. Scheduling gates — намеренно **внешний** механизм: kube-scheduler видит под с непустым
   `spec.schedulingGates` и просто его игнорирует, ничего не проверяя. Снять gate может только
   тот, кто его поставил (контроллер вроде Kueue или человек патчем) — иначе смысл «ждём
   внешнего решения» терялся бы. Добавить gate после создания пода, кстати, нельзя — только
   удалить.
...
6. Через какой механизм (subresource) API сервера выполняется вертикальное масштабирование ресурсов на лету? Почему нельзя просто обновить поле `spec`?
   ...
   Через subresource **`resize`** (`PATCH /api/v1/namespaces/.../pods/<name>/resize`).
   Обычный PATCH на `spec` отклоняется: поля ресурсов пода исторически иммутабельны, а
   отдельный subresource позволяет выдать право на изменение размеров, не давая права менять
   остальную спецификацию, и даёт kubelet точку для проверки выполнимости (`Infeasible`).
...
7. Объясните причину, по которой для CPU рекомендуется устанавливать `resizePolicy: NotRequired`, а для Memory — `RestartContainer`. Как ведут себя популярные рантаймы (например, JVM) при изменении лимита памяти на лету?
   ...
   CPU можно менять на лету: cgroup просто получает новую квоту, процесс этого даже не
   заметит — поэтому `NotRequired` (без рестарта). Память сжимается небезопасно: уже
   выделенные страницы никуда не денутся, и снижение `memory.max` приведёт к OOM. JVM
   дополнительно вычисляет размер кучи (`MaxRAMPercentage`) **при старте** и не пересматривает
   его — увеличение лимита на лету она не заметит, а уменьшение сделает OOM почти
   неизбежным. Отсюда `RestartContainer` для памяти.
...
8. Почему Kubernetes запрещает изменять QoS-класс пода в процессе in-place resize?
   ...
   QoS-класс вычисляется из соотношения requests/limits и определяет, как под ведёт себя при
   давлении на ноду (порядок выселения, `oom_score_adj`). Смена класса на лету означала бы, что
   под, принятый на ноду как `Guaranteed`, внезапно становится `Burstable` (или наоборот) —
   менялись бы гарантии, уже учтённые планировщиком и kubelet. Проще запретить: попытка даёт
   ошибку `QOS Class may not change`.
...
9. Если под находится в фазе `Pending`, как отличить ситуацию нехватки ресурсов от применения Scheduling Gates?
   ...
   Смотреть `status.conditions[type=PodScheduled]`: при нехватке ресурсов будет
   `reason: Unschedulable` и событие `FailedScheduling` с текстом (`Insufficient cpu`);
   при gate — `reason: SchedulingGated`, а событий планирования **нет вообще** (планировщик
   под не рассматривал). Ещё быстрее: колонка STATUS показывает `SchedulingGated`, а
   `spec.schedulingGates` непуст (сценарий 02).
...
10. Что означает статус resize `Infeasible` и в каком случае kubelet может установить его?
   ...
   `Infeasible` означает, что kubelet не может выполнить запрошенный resize на **этой** ноде:
   запрошено больше, чем у ноды есть свободного (allocatable минус уже выделенное другим
   подам). Под продолжает работать со старыми значениями, а статус остаётся `Infeasible`, пока
   запрос не уменьшат или под не переедет на другую ноду. Отличается от `Deferred` —
   временной невозможности, которую kubelet будет повторять.
