...
<!-- NAV -->
**⬅ [21-stateful-systems](../21-stateful-systems/)** · [индекс курса](../../README.md) · [карта обучения](../../docs/02-learning-path.md) · **[23-gateway-api](../23-gateway-api/) ➡**
<!-- /NAV -->
...
1. Каков полный жизненный цикл запроса от браузера пользователя до пода бэкенда при использовании Ingress? Какие компоненты Kubernetes участвуют в этом пути?
   ...
   Браузер резолвит имя в IP, за которым стоит вход в кластер (LoadBalancer или NodePort ноды),
   трафик попадает в под **Ingress-контроллера**. Контроллер (а не сам объект Ingress) читает
   правила из объектов Ingress через apiserver и держит на их основе конфигурацию nginx.
   По `Host` и пути он выбирает backend, берёт адреса Ready-подов из EndpointSlice
   соответствующего Service и проксирует запрос напрямую в под. Объект `Ingress` — только
   декларация; работу делают контроллер, Service (как источник endpoints) и CNI.
...
2. В чём заключается концептуальная разница между маршрутизацией на основе хоста (Host-based) и маршрутизацией на основе пути (Path-based)? Можно ли их комбинировать в одном Ingress-ресурсе?
   ...
   Host-based разводит трафик по доменам (`a.lab.local` → web-a, `b.lab.local` → web-b) —
   это разные «сайты» на одном входе. Path-based разводит по префиксу пути внутри одного домена
   (`/a` → web-a, `/b` → web-b) — это один сайт из нескольких сервисов. Комбинировать можно:
   в одном Ingress перечисляются правила с разными `host`, внутри каждого — несколько `paths`.
...
3. Объясните своими словами, для чего нужна аннотация `nginx.ingress.kubernetes.io/rewrite-target`. Что произойдет, если направить запрос `/frontend/css/style.css` на бэкенд без этой аннотации?
   ...
   Ingress передаёт бэкенду путь **как есть**. Если внешний путь `/frontend/...` — это лишь
   префикс маршрутизации, а приложение внутри ожидает `/css/style.css`, оно ответит 404.
   `rewrite-target` переписывает путь перед отправкой в бэкенд (обычно в связке с regex-группой
   в `path`), «срезая» префикс. Без неё запрос `/frontend/css/style.css` уйдёт в под именно с
   этим путём — и nginx-бэкенд будет искать несуществующий каталог `frontend`.
...
4. Объясните механизм TLS termination на Ingress-контроллере. Зачем терминировать TLS на балансировщике, а не передавать зашифрованный трафик напрямую в поды?
   ...
   Контроллер расшифровывает TLS у себя и дальше идёт по HTTP в кластерной сети. Плюсы:
   сертификаты хранятся и обновляются в одном месте (не нужно раздавать их в каждый под),
   контроллер видит HTTP и может маршрутизировать по пути/заголовкам, писать access-логи,
   считать метрики, делать rewrite и rate-limit; разгружается CPU приложений. Если требуется
   шифрование и внутри (zero-trust), добавляют mTLS через service mesh или
...
5. Как Ingress-контроллер определяет, какой именно сертификат предоставить клиенту во время TLS рукопожатия, если на одном IP-адресе (контроллере) обслуживается 100 разных доменов? Какая технология за это отвечает?
   ...
   **SNI (Server Name Indication)** — расширение TLS: клиент в ClientHello передаёт имя хоста
   открытым текстом, и контроллер по нему выбирает нужный сертификат из своих Secret'ов до
   завершения рукопожатия. Без SNI на одном IP можно было бы держать только один сертификат
   (или wildcard).
...
6. Почему браузеры и утилита `curl` по умолчанию выдают ошибку безопасности при подключении к нашему домену `secure.lab.local`?
   ...
   Потому что сертификат самоподписанный: его выпустил наш собственный `selfsigned-issuer`, а
   не доверенный публичный CA, и в хранилище доверенных корней такого нет. Плюс имя
   `secure.lab.local` не резолвится публично — в лабе его подставляют через `--resolve`/`Host`.
   Для тестов используют `curl -k` (не проверять цепочку) или добавляют CA в доверенные.
...
7. Какую роль в архитектуре cert-manager выполняют ресурсы `Issuer` (или `ClusterIssuer`) и `Certificate`? Чем они отличаются друг от друга?
   ...
   `Issuer`/`ClusterIssuer` — **кто выпускает**: конфигурация центра сертификации (selfsigned,
   CA, ACME/Let's Encrypt, Vault). Разница между ними только в области видимости: `Issuer`
   работает в своём namespace, `ClusterIssuer` — во всём кластере. `Certificate` — **что
   выпустить**: имена (dnsNames), срок, ссылка на issuer и имя Secret'а, куда положить ключ и
   сертификат. cert-manager следит за `Certificate` и перевыпускает его до истечения.
...
8. Как работает механизм ingress-shim? Опишите, как добавление одной строки аннотации в Ingress приводит к появлению готового сертификата.
   ...
   ingress-shim — контроллер внутри cert-manager, наблюдающий за объектами Ingress. Увидев
   аннотацию `cert-manager.io/cluster-issuer: <issuer>` и секцию `spec.tls` с `secretName` и
   `hosts`, он **сам создаёт** объект `Certificate` с этими именами. Дальше обычный поток
   cert-manager: заказ у issuer (для ACME — с решением challenge), запись ключа и сертификата
   в указанный Secret. Ingress-контроллер подхватывает Secret и начинает отдавать по нему TLS.
...
9. Вы выполнили команду `kubectl get ingress` и видите, что поле `ADDRESS` пустое уже 5 минут. Назовите две самые вероятные причины такого поведения.
   ...
   (1) Ingress не взят ни одним контроллером: не указан или неверен `ingressClassName` (или нет
   IngressClass по умолчанию) — объект просто никем не обрабатывается. (2) Сам контроллер не
   работает: подов нет/не Ready, либо его Service типа LoadBalancer висит в `<pending>` на
   bare-metal (без MetalLB). Реже — контроллер запущен без флага публикации адреса
   (`--publish-service`/`--report-node-internal-ip-address`).
...
10. Ваш запрос к Ingress возвращает `404 Not Found` от NGINX. Как вы будете искать причину этой ошибки?
   ...
   404 от nginx означает: запрос дошёл до контроллера, но правило не подобралось. Проверяем:
   совпадает ли `Host` в запросе с `spec.rules[].host` (curl с `-H 'Host: ...'` или
   `--resolve`); совпадает ли путь с учётом `pathType` (`Prefix` против `Exact`) и не нужен ли
   `rewrite-target`; тот ли namespace (Ingress ищет Service в своём); наконец
   `kubectl -n ingress-nginx logs deploy/ingress-nginx-controller` — там видно, какой upstream
   выбран. Если бы правило подобралось, но бэкенд был пуст, мы получили бы 503, а не 404
   (сценарий 02).
