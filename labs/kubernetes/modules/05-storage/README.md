...
> ⏱ время ~75 мин · сложность 2/5 · пререквизиты: модуль 03

<!-- NAV -->
**⬅ [04-networking](../04-networking/)** · [индекс курса](../../README.md) · [карта обучения](../../docs/02-learning-path.md) · **[06-scheduling](../06-scheduling/) ➡**
<!-- /NAV -->
...
1. Перечислите уровни жизни данных в k8s (контейнер → Pod → нода → кластер) и сопоставьте с типами томов.
   ...
   Жизнь данных: **контейнер** — слой образа и запись в него (исчезает при рестарте контейнера);
   **Pod** — `emptyDir` (исчезает вместе с подом); **нода** — `hostPath`/local PV (данные привязаны
   к конкретной ноде, переживают под, но не переезд); **кластер** — PVC поверх сетевого/CSI
   хранилища (переживают под и ноду). Выбирать надо по тому, какой из этих уровней потери
   приемлем.
...
2. Почему `emptyDir` нельзя использовать для БД, даже если приложение «вроде работает»?
   ...
   Потому что `emptyDir` живёт ровно столько, сколько живёт под: любой rollout, вытеснение,
   переезд на другую ноду или `delete pod` стирает данные — а приложение при этом не выдаёт
   ошибок, и в мониторинге всё зелёное. «Вроде работает» до первого пересоздания пода (см.
   `broken/scenario-03`).
...
3. В каких сценариях `hostPath` оправдан, несмотря на риски? (Подсказка: DaemonSet-агенты.)
   ...
   Когда под по назначению работает с самой нодой: агенты сбора логов (`/var/log/pods`),
   экспортёры метрик ноды, CSI/CNI-компоненты, системные DaemonSet. Такие поды привязаны к ноде
   осознанно, и `hostPath` для них — не костыль, а интерфейс к хосту. Для приложений это
   анти-паттерн: привязка к ноде, обход квот и прямой доступ к ФС хоста.
...
4. Опишите полный путь динамического provisioning от `kubectl apply -f pvc.yaml` до смонтированного тома.
   ...
   `apply pvc.yaml` → PVC в `Pending` → контроллер видит `storageClassName` и вызывает
   provisioner (CSI external-provisioner) → тот создаёт том в бэкенде и объект PV → PV
   связывается с PVC (`Bound`) → под, использующий PVC, планируется на ноду → kubelet через CSI
   делает `NodeStageVolume`/`NodePublishVolume` (форматирование и монтирование) → том появляется
   в контейнере по `mountPath`. При `WaitForFirstConsumer` всё начинается с планирования пода.
...
5. Чем статическое provisioning отличается от динамического? Кто создаёт PV в каждом случае?
   ...
   Статическое: администратор заранее создаёт объекты PV, PVC потом «подбирает» подходящий по
   классу, размеру и accessModes. Динамическое: PV не существует, его создаёт provisioner в
   момент появления PVC. В первом случае PV создаёт человек, во втором — контроллер по
   StorageClass.
...
6. Что произойдёт с PVC без `storageClassName`, если в кластере нет дефолтного класса?
   ...
   PVC останется в `Pending` навсегда: без явного класса используется дефолтный, а его нет —
   никто не возьмётся создавать том. В событиях PVC будет `no persistent volumes available for
   this claim and no storage class is set`. Лечится либо дефолтным классом, либо явным
   `storageClassName`.
...
7. Зачем нужен `WaitForFirstConsumer` и какую проблему `Immediate` он решает на зональных кластерах?
   ...
   `Immediate` создаёт том сразу при появлении PVC — в зональном облаке том может оказаться в
   зоне A, а под потом спланируется в зону B и никогда не запустится.
   `WaitForFirstConsumer` откладывает создание тома до момента, когда scheduler выбрал ноду, и
   создаёт его в правильной зоне/на правильной ноде. Для local-path на стенде это обязательное
   поведение.
...
8. Сравните `reclaimPolicy: Delete` и `Retain` с точки зрения сохранности данных и операционных рисков.
   ...
   `Delete` — при удалении PVC удаляется и PV, и данные в бэкенде: удобно для эфемерных
   окружений, опасно для прода (случайный `delete pvc` = потеря данных). `Retain` — PV
   сохраняется в состоянии `Released`, данные целы, но том не переиспользуется автоматически:
   нужен ручной шаг администратора. Для важных данных — `Retain` плюс бэкап.
...
9. Почему `ReadWriteOnce` — это «одна нода», а не «один Pod»? Когда нужен `ReadWriteOncePod`?
   ...
   Ограничение накладывается на **монтирование тома нодой**: ReadWriteOnce разрешает
   одновременное монтирование только одной ноде, но на этой ноде том могут использовать
   несколько подов сразу. Если нужно «строго один под», с 1.22 есть `ReadWriteOncePod` —
   тогда даже соседний под на той же ноде получит отказ.
...
10. Какой `accessMode` нужен, чтобы один том писали поды с разных нод, и что для этого требуется от провижинера?
   ...
   `ReadWriteMany`. Провижинер должен поддерживать разделяемый доступ: NFS, CephFS, Azure
   Files, GlusterFS. Блочные тома (EBS, GCE PD, local-path) этого не умеют — им нужен
   кластер-ФС поверх или другой драйвер.
...
11. Опишите механику ошибки `Multi-Attach` и как её избежать архитектурно.
   ...
   RWO-том примонтирован к ноде A. Под пересоздаётся и попадает на ноду B; том нельзя
   примонтировать второй ноде, пока он не отцеплен от первой — `attachdetach`-контроллер ждёт,
   и под висит в `ContainerCreating` с `Multi-Attach error for volume`. Часто это случается при
   `RollingUpdate`, когда новый под поднимается до удаления старого. Архитектурно: использовать
   `strategy: Recreate` для однорепликовых нагрузок с RWO или StatefulSet, где у каждой реплики
   свой том.
...
12. Чем StatefulSet принципиально отличается от Deployment по идентичности и хранилищу?
   ...
   Deployment считает поды взаимозаменяемыми: случайные имена, общий том (или никакого).
   StatefulSet даёт стабильные имя и порядковый номер, стабильное DNS-имя через headless Service
   и **собственный PVC на каждую реплику** из `volumeClaimTemplates` — то есть идентичность,
   переживающую пересоздание.
...
13. Как именуются PVC из `volumeClaimTemplates` и почему это важно для восстановления состояния?
   ...
   `<имя тома из шаблона>-<имя StatefulSet>-<порядковый номер>`, например `data-web-0`. Имя
   детерминировано, поэтому пересозданный `web-0` снова получает **свой** том с прежними
   данными; это же позволяет восстановить состояние, пересоздав StatefulSet с тем же именем.
...
14. Что происходит с PVC при scale down и при удалении StatefulSet? Как управлять этим в k8s ≥ 1.27?
   ...
   По умолчанию PVC не удаляются ни при scale down, ни при удалении StatefulSet — чтобы не
   потерять данные (обратное масштабирование вернёт под к его тому). С 1.27 поведением
   управляет `persistentVolumeClaimRetentionPolicy` с полями `whenScaled` и `whenDeleted`
   (`Retain` по умолчанию, можно `Delete`).
...
15. Зачем StatefulSet нужен headless Service?
   ...
   Чтобы у каждой реплики было стабильное DNS-имя `<pod>.<svc>.<ns>.svc.cluster.local`.
   Обычный Service дал бы один ClusterIP и случайную балансировку, а stateful-клиентам нужно
   обращаться к конкретному экземпляру (primary БД, конкретный брокер) и знать список peers.
...
16. PVC в `Pending`. Назовите минимум три разные причины и команду диагностики для каждой.
   ...
   (1) Нет StorageClass или опечатка в имени — `kubectl describe pvc` покажет
   `storageclass ... not found`. (2) `WaitForFirstConsumer` и ни один под ещё не использует PVC —
   в событиях `waiting for first consumer to be created`; это не ошибка. (3) Провижинер есть, но
   не смог создать том (нет места, нет прав, квота бэкенда) — `ProvisioningFailed` в событиях
   PVC и логи контроллера провижинера. Для статических PV — нет подходящего `Available` PV по
   классу/размеру/accessModes.
...
17. Pod застрял в `ContainerCreating` с `FailedMount`. Куда смотреть и какова типовая первопричина?
   ...
   `kubectl describe pod` → события: `FailedAttachVolume`/`Multi-Attach` (том ещё держит другая
   нода), `FailedMount ... timeout` (проблема CSI node-plugin, драйвера ФС или сети до СХД),
   `MountVolume.SetUp failed ... not found` (нет Secret/ConfigMap для projected-тома). Типовая
   первопричина в лабе — RWO-том, который не отцепился от старой ноды.
...
18. Можно ли изменить `storageClassName` или уменьшить размер у существующего PVC? Почему?
   ...
   Нет. `storageClassName` и большинство полей PVC иммутабельны — PVC надо пересоздать (данные
   при `reclaimPolicy: Retain` можно переиспользовать, привязав новый PVC к существующему PV).
   Размер можно только **увеличить**, и только если у StorageClass `allowVolumeExpansion: true`;
   уменьшение не поддерживается ни одним драйвером — файловые системы не умеют безопасно
   сжиматься на лету.
...
19. Как безопасно «вернуть в строй» `Released` PV без потери данных?
   ...
   `Released` означает: PVC удалён, данные на PV остались, но PV не готов к новому claim.
   Безопасный путь: снять у PV `spec.claimRef` (`kubectl patch pv <pv> -p
   '{"spec":{"claimRef": null}}'`) — PV перейдёт в `Available` и сможет связаться с новым PVC
   того же класса и размера. Сам PV при этом не удаляют, данные не трогают.
...
20. Как изнутри Pod проверить реальный размер и заполнение примонтированного PV?
   ...
   `kubectl -n lab exec <pod> -- df -h <mountPath>` — покажет реальный размер файловой системы
   и заполнение. Полезно сверить с `spec.resources.requests.storage`: у некоторых провижинеров
   (в том числе local-path) фактический размер равен размеру раздела ноды, а «запрошенный»
   объём — только учётная величина.
