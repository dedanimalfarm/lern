...
- [Методология (один цикл на любой инцидент)](#методология-один-цикл-на-любой-инцидент)
- [Каталог инцидентов](#каталог-инцидентов)
- [Дерево триажа (по симптому)](#дерево-триажа-по-симптому)
- [Сценарий 5: DNS failure](#сценарий-5-dns-failure)
- [Сценарий 6: Pending — под не размещается](#сценарий-6-pending--под-не-размещается)
- [Сценарий 7: GitOps sync-fail (Argo CD)](#сценарий-7-gitops-sync-fail-argo-cd)
- [Сценарий 8: Cert-expiry (протухший TLS)](#сценарий-8-cert-expiry-протухший-tls)
- [Авто-триаж — `triage/incident-triage.sh`](#авто-триаж--triageincident-triagesh)
- [Случайный инцидент — `chaos/random-incident.sh` (режим экзамена)](#случайный-инцидент--chaosrandom-incidentsh-режим-экзамена)
...
> ⏱ время ~90 мин · сложность 5/5 · пререквизиты: Трек 1 и Трек 2

<!-- NAV -->
**⬅ [project-e-secure-platform](../project-e-secure-platform/)** · [индекс курса](../../README.md) · [карта обучения](../../docs/02-learning-path.md) · **конец ➡**
<!-- /NAV -->
...
## Случайный инцидент — `chaos/random-incident.sh` (режим экзамена)

Статичные сценарии быстро запоминаются: через пару прогонов вы «узнаёте» поломку по имени
файла, а не по симптому. Скрипт выбирает инциденты случайно и не говорит, какие:

```bash
bash chaos/random-incident.sh start 2      # применить два случайных инцидента из каталога 1–8
kubectl -n lab get pods                     # искать симптомы, как в проде
bash triage/incident-triage.sh lab app=<label>
bash chaos/random-incident.sh reveal       # свериться с ответом ПОСЛЕ диагноза
bash chaos/random-incident.sh reset        # убрать всё, что применил start
```

Сценарий 7 (Argo sync-fail) попадает в выборку только если в кластере есть Argo CD;
переменная `NS` меняет namespace, `CHAOS_STATE` — файл с ответом.
...
1. Запустите `chaos/random-incident.sh start 3` и локализуйте все три причины только
   через `triage/incident-triage.sh` + `describe`, не открывая `reveal` до диагноза.
