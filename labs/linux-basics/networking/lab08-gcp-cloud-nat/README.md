...
1. **Вопрос:** В чем разница между Cloud NAT в GCP и традиционным NAT-инстансом (NAT VM)?
   **Ответ:** Cloud NAT — это полностью управляемый, распределенный программно-определяемый сервис (software-defined). Он не создает единую точку отказа и не требует развертывания отдельной виртуальной машины, которая тратит ресурсы и требует обслуживания. Трафик не проходит через промежуточный хост-посредник, а транслируется на уровне программной сети Andromeda.
2. **Вопрос:** Что такое исчерпание портов (port exhaustion) в Cloud NAT и как с этим бороться?
   **Ответ:** Каждой виртуальной машине выделяется определенное фиксированное количество портов источника (по умолчанию 64) для связи с внешними ресурсами. Если VM генерирует множество параллельных сессий к одному и тому же IP-адресу и порту назначения, доступные порты могут закончиться, и новые соединения будут отбрасываться. Решить проблему можно увеличением минимального числа портов на VM (например, до 128 или 256), включением функции динамического выделения портов (Dynamic Port Allocation) или бронированием нескольких статических IP-адресов под NAT.
3. **Вопрос:** Зачем нужен Cloud Router, если Cloud NAT не использует виртуальные машины для маршрутизации трафика?
   **Ответ:** Cloud Router является управляющим звеном (control plane). Он не маршрутизирует пакеты данных (data plane), но используется для настройки параметров конфигурации и динамического обновления таблиц трансляции в программной сети GCP.
