...
- [Часть 1: Что такое seccomp](#часть-1-что-такое-seccomp)
  - [Теория для изучения перед частью](#теория-для-изучения-перед-частью)
  - [1.1 Посмотреть seccomp-статус процесса](#11-посмотреть-seccomp-статус-процесса)
- [Часть 2: Свой seccomp-bpf руками (prctl)](#часть-2-свой-seccomp-bpf-руками-prctl)
  - [Теория для изучения перед частью](#теория-для-изучения-перед-частью-1)
  - [2.1 Заблокировать uname (syscall 63) и убедиться в SIGSYS](#21-заблокировать-uname-syscall-63-и-убедиться-в-sigsys)
- [Часть 3: Быстрый путь — systemd-run](#часть-3-быстрый-путь--systemd-run)
  - [Теория для изучения перед частью](#теория-для-изучения-перед-частью-2)
  - [3.1 Запретить syscall и класс через systemd-run](#31-запретить-syscall-и-класс-через-systemd-run)
- [Часть 4: Troubleshooting](#часть-4-troubleshooting)
  - [Теория: диагностика по симптому](#теория-диагностика-по-симптому)
  - [Инцидент 1: не-root не может поставить фильтр без `PR_SET_NO_NEW_PRIVS`](#инцидент-1-не-root-не-может-поставить-фильтр-без-pr_set_no_new_privs)
