...
- [Часть 1: Шесть namespace по одному](#часть-1-шесть-namespace-по-одному)
  - [Теория для изучения перед частью](#теория-для-изучения-перед-частью)
  - [1.1 Поднять каждый namespace и сравнить inode](#11-поднять-каждый-namespace-и-сравнить-inode)
- [Часть 2: Эффект каждого namespace](#часть-2-эффект-каждого-namespace)
...
- [Часть 3: Всё сразу — это и есть `docker run`](#часть-3-всё-сразу--это-и-есть-docker-run)
  - [Теория для изучения перед частью](#теория-для-изучения-перед-частью-2)
  - [3.1 Базовый контейнер одной командой](#31-базовый-контейнер-одной-командой)
- [Часть 4: Troubleshooting](#часть-4-troubleshooting)
  - [Теория: диагностика по симптому](#теория-диагностика-по-симптому)
  - [Инцидент 1: новый PID-namespace, но `ps` врёт / `$$` не 1](#инцидент-1-новый-pid-namespace-но-ps-врёт---не-1)
