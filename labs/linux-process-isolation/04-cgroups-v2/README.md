...
- [Часть 1: Устройство cgroup v2 и размещение процесса](#часть-1-устройство-cgroup-v2-и-размещение-процесса)
  - [Теория для изучения перед частью](#теория-для-изучения-перед-частью)
  - [1.1 Создать cgroup и включить контроллеры](#11-создать-cgroup-и-включить-контроллеры)
- [Часть 2: Лимит CPU (cpu.max → throttling)](#часть-2-лимит-cpu-cpumax--throttling)
  - [Теория для изучения перед частью](#теория-для-изучения-перед-частью-1)
  - [2.1 Ограничить busy-loop 20% ядра](#21-ограничить-busy-loop-20-ядра)
- [Часть 3: Лимит памяти (memory.max → OOM-kill)](#часть-3-лимит-памяти-memorymax--oom-kill)
  - [Теория для изучения перед частью](#теория-для-изучения-перед-частью-2)
  - [3.1 Поймать OOM в лимите 50M](#31-поймать-oom-в-лимите-50m)
- [Часть 4: Лимит PIDs (pids.max → отказ fork)](#часть-4-лимит-pids-pidsmax--отказ-fork)
  - [Теория для изучения перед частью](#теория-для-изучения-перед-частью-3)
  - [4.1 Упереться в pids.max=5](#41-упереться-в-pidsmax5)
- [Часть 5: Troubleshooting](#часть-5-troubleshooting)
  - [Теория: диагностика по симптому](#теория-диагностика-по-симптому)
  - [Инцидент 1: в дочерней cgroup нет `cpu.max`](#инцидент-1-в-дочерней-cgroup-нет-cpumax)
