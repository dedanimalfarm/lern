...
## ⚠️ Где проводилось тестирование

Лабораторная работа полностью прогнана на:

- **Облако:** AWS, account `dedanimalfarm` (`537053563773`), регион **eu-north-1** (Stockholm).
- **Инстанс:** `i-096fc64e179ee8a93`, тип `t3.micro` (2 vCPU, 1 GiB RAM, 8 GiB gp3).
- **AMI:** `ami-059f32cf6eecf0ef9` — **Amazon Linux 2023**, ядро 6.1.x.
- **Подключение:** `ssh -i ~/.ssh/aws-dedanimalfarm ec2-user@13.62.58.209`.
...
> 🛡️ **Безопасность:** `chroot` не является границей безопасности. С root-правами
> и доступом к `/proc` процесс выходит к корню хоста через `/proc/1/root`
> (см. §5). Работайте на **отдельной тестовой ВМ** — у нас это и есть `t3.micro`.
